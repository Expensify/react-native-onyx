...
describe('OnyxUtils', () => {
    ...
    describe('mergeCollection cache-only reads', () => {
        ...
        it('skips storage reads entirely — the previous value comes from the cache', async () => {
            ...
            // Reads are synchronous and cache-only now, so mergeCollection never touches storage
            // to resolve the previous value — only the subsequent writes hit storage.
        ...
        it('subscriber receives a single merged broadcast for an Onyx.update batch (no transient undefined)', async () => {
            const collectionKey = ONYXKEYS.COLLECTION.TEST_KEY;
            ...
            // The cache read resolves synchronously, so the Onyx.update batch broadcasts exactly
