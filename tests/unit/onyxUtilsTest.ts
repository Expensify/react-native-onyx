...
describe("OnyxUtils", () => {
  beforeAll(() =>
    Onyx.init({
      keys: ONYXKEYS,
      ramOnlyKeys: [
        ONYXKEYS.RAM_ONLY_KEY,
        ONYXKEYS.COLLECTION.RAM_ONLY_COLLECTION,
      ],
    }),
  );
  ...
  describe("multiSetWithRetry", () => {
    ...
    it("should fire individual member-key subscribers once per key", async () => {
      ...
      expect(spy1).toHaveBeenCalledWith(
        { id: 1 },
        `${ONYXKEYS.COLLECTION.TEST_KEY}1`,
      );
      expect(spy2).toHaveBeenCalledTimes(1);
      expect(spy2).toHaveBeenCalledWith(
        { id: 2 },
        `${ONYXKEYS.COLLECTION.TEST_KEY}2`,
      );
      expect(spy3).toHaveBeenCalledTimes(1);
      expect(spy3).toHaveBeenCalledWith(
        { id: 3 },
        `${ONYXKEYS.COLLECTION.TEST_KEY}3`,
      );
    ...
    it("should notify non-collection keys individually alongside batched collection updates", async () => {
      ...
      expect(singleKeyCallback).toHaveBeenCalledWith(
        "standalone",
        ONYXKEYS.TEST_KEY,
      );
  ...
  describe("keysChanged", () => {
    ...
    it("should notify collection-level subscribers with the whole collection object", async () => {
      ...
      const [receivedCollection, receivedKey] =
        collectionCallback.mock.calls.at(0);
    ...
    it("should notify member subscribers only for changed keys in a batched update", async () => {
      ...
      OnyxUtils.keysChanged(
        ONYXKEYS.COLLECTION.TEST_KEY,
        { [keyA]: newA, [keyB]: dataB, [keyC]: newC },
  ...
  describe("mergeAndMarkChanges", () => {
    it("should apply the replacement markers if we have properties with objects being removed and added back during the changes", () => {
      const { result, replaceNullPatches } =
        OnyxUtils.mergeAndMarkChanges(testMergeChanges);
  ...
  describe("retryOperation", () => {
    ...
    const invalidDataError = new Error(
      "Failed to execute 'put' on 'IDBObjectStore': invalid data",
    );
    const diskFullError = new Error("database or disk is full");
    const nonRetriableIdbError = Object.assign(
    ...
    it("should retry only one time if the operation is firstly failed and then passed", async () => {
      StorageMock.setItem = jest
        .fn(StorageMock.setItem)
        .mockRejectedValueOnce(genericError)
        .mockImplementation(StorageMock.setItem);
    ...
    it("should log the full shape of an unclassified (UNKNOWN) error once per operation", async () => {
      ...
      const unclassifiedCalls = logAlertSpy.mock.calls.filter(
        (call) =>
          typeof call[0] === "string" &&
          call[0].startsWith("Unclassified storage error."),
      );
    ...
    it("should throw error for if operation failed with \"Failed to execute 'put' on 'IDBObjectStore': invalid data\" error", async () => {
      ...
      await expect(
        Onyx.set(ONYXKEYS.TEST_KEY, { test: "data" }),
    ...
    it("should include the error in logAlert for IDBObjectStore invalid data errors", async () => {
      ...
      expect(logAlertSpy).toHaveBeenCalledWith(
        `Attempted to set invalid data set in Onyx. Please ensure all data is serializable. Error: ${invalidDataError}`,
      );
    });
    ...
    it("should include the error in logs when out of storage with no evictable keys", async () => {
      ...
      expect(logAlertSpy).toHaveBeenCalledWith(
        `Out of storage. But found no acceptable keys to remove. Error: ${diskFullError}`,
      );
      expect(logInfoSpy).toHaveBeenCalledWith(
    ...
    it("should include the error in logAlert when out of storage and getDatabaseSize fails", async () => {
      ...
      expect(logAlertSpy).toHaveBeenCalledWith(
        `Unable to get database size. getDatabaseSize error: ${dbSizeError}. Original error: ${diskFullError}`,
      );
    });
    ...
    it("should trip the circuit breaker and alert once after sustained capacity failures", async () => {
      ...
      expect(logAlertSpy).toHaveBeenCalledWith(
        expect.stringContaining("Storage circuit breaker tripped"),
      );
    });
  ...
  describe("mergeCollection cache-first ordering", () => {
    ...
    it("updates cache and notifies subscribers even when Storage.multiMerge rejects", async () => {
      ...
      StorageMock.multiMerge = jest
        .fn()
        .mockRejectedValue(nonRetriableIdbError);
      ...
      const lastBroadcast = collectionCallback.mock.calls.at(-1)?.[0] as
  ...
  describe("mergeCollection pre-warm", () => {
    ...
    it("equivalence: warm-path and cold-path produce the same final cache state for the same merge", async () => {
      ...
      const warmResult =
        OnyxCache.getCollectionData(collectionKey)?.[memberKey];
  ...
  describe("storage eviction", () => {
    ...
    it("should evict the least recently accessed evictable key on storage capacity error and retry successfully", async () => {
      ...
      LocalStorageMock.setItem = jest
        .fn(LocalStorageMock.setItem)
        .mockRejectedValueOnce(diskFullError)
        .mockImplementation(LocalStorageMock.setItem);
    ...
    it("should recover via a half-open eviction+retry probe after the open window clears", async () => {
      ...
      const LocalStorageCircuitBreaker =
        require("../../lib/StorageCircuitBreaker")
    ...
    it("should seed evictable keys from storage at init", async () => {
      ...
      expect(keyForEviction?.startsWith(ONYXKEYS.COLLECTION.TEST_KEY)).toBe(
        true,
      );
    });
    ...
    it("should include the error in logs when evicting a key", async () => {
      ...
      expect(logInfoSpy).toHaveBeenCalledWith(
        `Out of storage. Evicting least recently accessed key (${key1}) and retrying. Error: ${diskFullError}`,
      );
      expect(logInfoSpy).toHaveBeenCalledWith(
        `Storage Quota Check -- bytesUsed: 0 originWideBytesRemaining (estimate, not per-DB headroom): Infinity. Original error: ${diskFullError}`,
      );
    ...
    it("multiSet — eviction of an UNRELATED key still notifies its subscribers (codex regression guard)", async () => {
      ...
      LocalStorageMock.multiSet = jest
        .fn(LocalStorageMock.multiSet)
        .mockRejectedValueOnce(diskFullError)
        .mockImplementation(LocalStorageMock.multiSet);
    ...
    it("mergeCollection — evicts an unrelated key, not the in-flight key, so its fields survive", async () => {
      ...
      LocalStorageMock.multiMerge = jest
        .fn(LocalStorageMock.multiMerge)
        .mockRejectedValueOnce(diskFullError)
        .mockImplementation(LocalStorageMock.multiMerge);
    ...
    it("mergeCollection — does not truncate the in-flight key when it is the only evictable key", async () => {
      ...
      LocalStorageMock.multiMerge = jest
        .fn(LocalStorageMock.multiMerge)
        .mockRejectedValue(diskFullError);
