...
const generateConnectionID =
  connectionManager.generateConnectionID.bind(connectionManager);
...
describe("OnyxConnectionManager", () => {
  ...
  describe("generateConnectionID", () => {
    it("should generate a stable connection ID", async () => {
      ...
      expect(connectionID).toEqual(
        `onyxKey=${ONYXKEYS.TEST_KEY},sessionID=${getSessionID()}`,
      );
    });
    ...
    it("should generate a stable, reusable connection ID for collection keys", async () => {
      ...
      expect(connectionID).toEqual(
        `onyxKey=${ONYXKEYS.COLLECTION.TEST_KEY},sessionID=${getSessionID()}`,
      );
    });
    ...
    it("should generate unique connection IDs if certain options are passed", async () => {
      ...
      expect(
        connectionID1.startsWith(
          `onyxKey=${ONYXKEYS.TEST_KEY},sessionID=${getSessionID()},uniqueID=`,
        ),
      ).toBeTruthy();
      expect(
        connectionID2.startsWith(
          `onyxKey=${ONYXKEYS.TEST_KEY},sessionID=${getSessionID()},uniqueID=`,
        ),
      ).toBeTruthy();
  ...
  describe("unsubscribeFromKey", () => {
    it("should clean up the correct subscription ID from lastConnectionCallbackData on disconnect", async () => {
      ...
      const subscriptionIdA = connectionsMap.get(
        connectionA.id,
      )?.subscriptionID;
      ...
      const numericDeleteArgs = deleteSpy.mock.calls
        .map((call) => call[0])
    ...
    it("should remove the subscription ID from onyxKeyToSubscriptionIDs on disconnect", async () => {
      ...
      const subscriptionIdB = connectionsMap.get(
        connectionB.id,
      )?.subscriptionID;
      ...
      const setCallsForKey = setSpy.mock.calls.filter(
        (call) => call[0] === ONYXKEYS.TEST_KEY,
      );
      expect(setCallsForKey.length).toBeGreaterThan(0);

      const updatedIDs = setCallsForKey.at(
        setCallsForKey.length - 1,
      )[1] as number[];
