    ...
    it('should properly set and merge when using mergeCollection', () => {
        ...
        return Onyx.mergeCollection(ONYX_KEYS.COLLECTION.TEST_KEY, {
            ...
            .then(() => {
                ...
                expect(mockCallback.mock.calls[0][1]).toEqual('test_');

                expect(mockCallback.mock.calls[1][0]).toEqual({ID: 123, value: 'one'});
                expect(mockCallback.mock.calls[1][1]).toEqual('test_1');

                expect(mockCallback.mock.calls[2][0]).toEqual({ID: 234, value: 'two'});
                expect(mockCallback.mock.calls[2][1]).toEqual('test_2');

                expect(mockCallback.mock.calls[3][0]).toEqual({ID: 345, value: 'three'});
                expect(mockCallback.mock.calls[3][1]).toEqual('test_3');

                expect(mockCallback.mock.calls[4][0]).toEqual({ID: 123, value: 'five'});
                expect(mockCallback.mock.calls[4][1]).toEqual('test_1');

                expect(mockCallback.mock.calls[5][0]).toEqual({ID: 234, value: 'four'});
                expect(mockCallback.mock.calls[5][1]).toEqual('test_2');

                expect(mockCallback.mock.calls[6][0]).toEqual({ID: 456, value: 'two'});
                expect(mockCallback.mock.calls[6][1]).toEqual('test_4');

                expect(mockCallback.mock.calls[7][0]).toEqual({ID: 567, value: 'one'});
                expect(mockCallback.mock.calls[7][1]).toEqual('test_5');
