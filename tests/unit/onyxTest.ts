...
describe("Onyx", () => {
  beforeAll(() => {
    Onyx.init({
      ...
      ramOnlyKeys: [
        ...
        ONYX_KEYS.RAM_ONLY_WITH_INITIAL_VALUE,
      ],
  ...
  it("should mergeCollection an object into an empty array (treating [] as {})", async () => {
    ...
    expect(
      (testKeyValue as Record<string, unknown>)?.[collectionKey],
    ).toStrictEqual([]);
    ...
    return Onyx.mergeCollection(ONYX_KEYS.COLLECTION.TEST_KEY, {
      ...
      .then(() => {
        ...
        expect(mockCallback).toHaveBeenNthCalledWith(
          2,
          {
            test_1: { ID: 123, value: "five" },
            test_2: { ID: 234, value: "four" },
            // test_3 unchanged (incompatible array merge rejected)
            test_3: { ID: 345, value: "three" },
            test_4: { ID: 456, value: "two" },
  ...
  it("should return all collection keys as a single object", () => {
    const mockCallback = jest.fn();
    ...
    return Onyx.mergeCollection(
      ONYX_KEYS.COLLECTION.TEST_CONNECT_COLLECTION,
      initialCollectionData as GenericCollection,
    )
      .then(() => {
        // When we connect to that collection
        connection = Onyx.connect({
      ...
      waitForPromisesToResolve()
        ...
        .then(() =>
          Onyx.mergeCollection(
            ONYX_KEYS.COLLECTION.TEST_POLICY,
            collectionUpdate as GenericCollection,
          ),
        )
        .then(() => {
  ...
  it("should send a value to Onyx.connect() when subscribing to a specific collection member key and keysChanged() is called", () => {
    ...
    // Given an Onyx.connect call subscribing to a single collection member key
    connection = Onyx.connect({
  ...
  it("should return all collection keys as a single object when a single collection member key is updated", () => {
    const mockCallback = jest.fn();
    const collectionUpdate = {
      testPolicy_1: { ID: 234, value: "one" },
    };
    ...
    return (
      waitForPromisesToResolve()
        ...
        .then(() =>
          Onyx.merge(
            `${ONYX_KEYS.COLLECTION.TEST_POLICY}${1}`,
            collectionUpdate.testPolicy_1,
          ),
        )
        .then(() => {
    ...
    return (
      waitForPromisesToResolve()
        ...
        .then(() =>
          Onyx.merge(
            `${ONYX_KEYS.COLLECTION.TEST_POLICY}${1}`,
            lodashClone(collectionUpdate.testPolicy_1),
          ),
        )
        .then(() => {
          // Then we should not expect another invocation of the callback
  ...
  it("should return a promise that completes when all update() operations are done", () => {
    ...
    connections.push(
      Onyx.connect({ key: ONYX_KEYS.TEST_KEY, callback: testCallback }),
    );
    connections.push(
      Onyx.connect({ key: ONYX_KEYS.OTHER_TEST, callback: otherTestCallback }),
    );
    connections.push(
      Onyx.connect({
    ...
    return waitForPromisesToResolve().then(() =>
      Onyx.update([
        ...
        expect(collectionCallback).toHaveBeenNthCalledWith(
          1,
          undefined,
          ONYX_KEYS.COLLECTION.TEST_UPDATE,
        );
        expect(collectionCallback).toHaveBeenNthCalledWith(
          2,
          { [itemKey]: { a: "a" } },
        ...
        expect(testCallback).toHaveBeenNthCalledWith(
          2,
          "taco",
          ONYX_KEYS.TEST_KEY,
        );
  ...
  it("should update Snapshot when its data changed", async () => {
    ...
    await Onyx.update([
      { key: cat, value: finalValue, onyxMethod: Onyx.METHOD.MERGE },
    ]);
  ...
  describe("update", () => {
    ...
    it("should return a promise that completes when all update() operations are done", () => {
      ...
      return Onyx.update([
        ...
        expect(catCallback).toHaveBeenNthCalledWith(
          1,
          { age: 3, sound: "meow" },
          cat,
        );
    ...
    it("should replace the old value after a null merge in the top-level object when batching updates", async () => {
      ...
      expect(
        await StorageMock.getItem(`${ONYX_KEYS.COLLECTION.TEST_UPDATE}entry1`),
      ).toEqual({ someKey: "someValueChanged" });
    });
    ...
    describe("should replace the old value after a null merge in a nested property when batching updates", () => {
      ...
      it("replacing old object after null merge", async () => {
        ...
        expect(
          await StorageMock.getItem(
            `${ONYX_KEYS.COLLECTION.TEST_UPDATE}entry1`,
          ),
        ).toEqual(entry1ExpectedResult);
      });
      ...
      it("setting new object after null merge", async () => {
        ...
        entry1ExpectedResult.sub_entry1.someNestedObject.anotherNestedObject2 =
          { anotherNestedKey2: "anotherNestedValue2" };
        ...
        delete entry1ExpectedResult.sub_entry1.someNestedObject
          .anotherNestedObject2;
      ...
      it("setting new object after null merge of a primitive property", async () => {
        ...
        delete entry1ExpectedResult.sub_entry1.someNestedObject
          .anotherNestedObject.anotherNestedKey;
        ...
        entry1ExpectedResult.sub_entry1.someNestedObject.anotherNestedObject.anotherNestedKey =
          { newNestedKey: "newNestedValue" };
      ...
      it("replacing nested object during updates", async () => {
        ...
        let entry1ExpectedResult = lodashCloneDeep(entry1) as
          | GenericDeepRecord
          | undefined;
        const queuedUpdates: Array<OnyxUpdate<OnyxKey>> = [];
    ...
    it("should trigger individual callbacks for each key when update is called with mergeCollection", async () => {
      ...
      expect(individualCallback1).toHaveBeenCalledWith(
        { id: "1", name: "Updated Item 1" },
        key1,
      );
      expect(individualCallback2).toHaveBeenCalledWith(
    ...
    it("should not save a RAM-only collection to storage", async () => {
      ...
      expect(cache.get(`${ONYX_KEYS.COLLECTION.RAM_ONLY_COLLECTION}1`)).toEqual(
        "updated test 1",
      );
      expect(cache.get(`${ONYX_KEYS.COLLECTION.RAM_ONLY_COLLECTION}2`)).toEqual(
        "updated test 2",
      );
      expect(
        await StorageMock.getItem(
          `${ONYX_KEYS.COLLECTION.RAM_ONLY_COLLECTION}1`,
        ),
      ).toBeNull();
      expect(
        await StorageMock.getItem(
          `${ONYX_KEYS.COLLECTION.RAM_ONLY_COLLECTION}2`,
        ),
      ).toBeNull();
    });
    ...
    describe("should log and skip invalid operations", () => {
      it("invalid method", async () => {
        ...
        expect(logInfoFn).toHaveBeenNthCalledWith(
          1,
          "Invalid onyxMethod invalidMethod in Onyx update. Skipping this operation.",
        );
      });
      ...
      it("non-object value passed to multiSet", async () => {
        ...
        expect(logInfoFn).toHaveBeenNthCalledWith(
          1,
          "Invalid value provided in Onyx multiSet. Value must be of type object. Skipping this operation.",
        );
      });
      ...
      it("non-string value passed to key", async () => {
        ...
        expect(logInfoFn).toHaveBeenNthCalledWith(
          1,
          "Invalid number key provided in Onyx update. Key must be of type string. Skipping this operation.",
        );
      });
      ...
      it("invalid or empty value passed to mergeCollection", async () => {
        ...
        expect(logInfoFn).toHaveBeenNthCalledWith(
          1,
          "Invalid or empty value provided in Onyx mergeCollection. Skipping this operation.",
        );
      });
    });
  });
  ...
  describe("multiSet", () => {
    it("should only save non RAM-only keys to storage", async () => {
      ...
      expect(await StorageMock.getItem(ONYX_KEYS.OTHER_TEST)).toEqual(
        otherTestValue,
      );
  ...
  describe("skippable collection member ids", () => {
    ...
    it("should clear pending merge for a key during multiSet()", async () => {
      ...
      jest
        .spyOn(OnyxUtils, "hasPendingMergeForKey")
        .mockImplementation((key) => key === testKey);
      jest
        .spyOn(OnyxUtils, "getMergeQueue")
        .mockImplementation(() => mockMergeQueue);
  ...
  describe("clear", () => {
    it("should handle RAM-only keys with defaults correctly during clear", async () => {
      ...
      expect(await StorageMock.getItem(ONYX_KEYS.RAM_ONLY_TEST_KEY)).toBeNull();
      expect(
        await StorageMock.getItem(ONYX_KEYS.RAM_ONLY_WITH_INITIAL_VALUE),
      ).toBeNull();
      // Verify cache state based on whether there's a default
      expect(cache.get(ONYX_KEYS.RAM_ONLY_TEST_KEY)).toBeUndefined();
      expect(cache.get(ONYX_KEYS.RAM_ONLY_WITH_INITIAL_VALUE)).toEqual(
        "default",
      );
    });
  });
...
describe("Onyx.init", () => {
  ...
  describe("should only execute Onyx methods after initialization", () => {
    ...
    it("multiSet", async () => {
      ...
      expect(
        cache.get(`${ONYX_KEYS.COLLECTION.TEST_KEY}entry1`),
      ).toBeUndefined();

      Onyx.init({ keys: ONYX_KEYS });
      await act(async () => waitForPromisesToResolve());

      expect(cache.get(`${ONYX_KEYS.COLLECTION.TEST_KEY}entry1`)).toEqual(
        "test_1",
      );
    });
    ...
    it("clear", async () => {
      ...
      const spyClearNullishStorageKeys = jest.spyOn(
        cache,
    ...
    it("update", async () => {
      Onyx.update([
        { onyxMethod: "set", key: ONYX_KEYS.TEST_KEY, value: "test" },
      ]);
      await act(async () => waitForPromisesToResolve());
...
describe("RAM-only keys should not read from storage", () => {
  ...
  it("should not include stale RAM-only keys in getAllKeys results", async () => {
    ...
    expect(keys.has(`${ONYX_KEYS.COLLECTION.RAM_ONLY_COLLECTION}1`)).toBe(
      false,
    );
  ...
  it("should not read stale storage data when subscribing to individual RAM-only collection members", async () => {
    ...
    expect(receivedValues.every((v) => v === undefined || v === null)).toBe(
      true,
    );
  ...
  it("should not sync RAM-only keys from other instances via keepInstancesSync", async () => {
    ...
    const syncCallback = (
      StorageMock.keepInstancesSync as jest.Mock
    ).mock.calls.at(0)?.[0];
