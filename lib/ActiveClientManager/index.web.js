    ...
    Broadcast.subscribe((message) => {
        switch (message.data.type) {
            case NEW_LEADER_MESSAGE: {
                ...
                // same, it used the client ID to tie-break
                const isCurrentTimestampSame = timestamp === message.data.timestamp;
                const isCurrentTimestampOlder = timestamp > message.data.timestamp;
                if (isClientTheLeader() && (isCurrentTimestampSame || (isCurrentTimestampOlder && clientID > message.data.clientID))) {
                    return;
                }
