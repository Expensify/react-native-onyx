...
class OnyxCache {
    ...
    /**
     * Check to see if the new collection we are merging will alter the value of the collection in the cache.
     *
     * @param {Object} partialCollection
     * @returns {Boolean}
     */
    hasCollectionValueChanged(partialCollection) {
        let hasChanged = false;
        const partialCollectionKeys = _.keys(partialCollection);
        for (let i = 0; i < partialCollectionKeys.length; i++) {
            const key = partialCollectionKeys[i];
            const oldValue = this.storageMap[key];
            const newValue = partialCollection[key];
