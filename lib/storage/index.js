...
const Storage = {
        ...
        initPromise = Promise.all(
            _.map(storageProviders.slice(1), (provider) => {
                try {
                    ...
                    Logger.logAlert(`${provider.name} could not be initialized: ${error.toString()}`);
            ...
            .then(() => {
                Logger.logInfo('Successfully initialized every storage provider.');
            ...
            .catch(() => {
                Logger.logAlert('Failed to initialize some of the storage providers. Falling back to memory-only storage provider');
            });
     ...
     */
    getItem(key) {
        return initPromise.finally(() => runConsecutively('getItem', key));
    },
     ...
     */
    multiGet(keys) {
        return initPromise.finally(() => runConsecutively('multiGet', keys));
    },
     ...
     */
    multiSet(pairs) {
        return initPromise.finally(() => runInParallel('multiSet', false, pairs));
    },
     ...
     */
    multiMerge(pairs) {
        return initPromise.finally(() => runInParallel('multiMerge', false, pairs));
    },
    ...
    setMemoryOnlyKeys() {
        return initPromise.finally(() => runInParallel('setMemoryOnlyKeys', true));
    },
     ...
     */
    getAllKeys() {
        return initPromise.finally(() => runConsecutively('getAllKeys'));
    },
     ...
     */
    getDatabaseSize() {
        return initPromise.finally(() => runConsecutively('getDatabaseSize'));
    },
