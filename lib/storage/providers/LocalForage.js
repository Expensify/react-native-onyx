...
const provider = {
     ...
     * to the next.
     */
    setItemQueue: new SyncQueue(({key, value}) => localforage.setItem(key, value)),
    multiMergeQueue: new SyncQueue((pairs) => {
        const tasks = _.map(pairs, ([key, partialValue]) => this.default.getItem(key)
            .then((existingValue) => {
                const newValue = _.isObject(existingValue)
                    ? lodashMerge({}, existingValue, partialValue)
                    : partialValue;
                return this.default.setItem(key, newValue);
            }));
     ...
     */
    multiMerge(pairs) {
        return this.multiMergeQueue.push(pairs);
    },
