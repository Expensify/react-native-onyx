...
const provider = {
     ...
     */
    applyMerge(key, data) {
        const mergeValues = this.mergeQueue[key];
     ...
     */
    mergeItem(key, value) {
        if (this.mergeQueue[key]) {
            this.mergeQueue[key].push(value);
            return Promise.resolve();
        }

        this.mergeQueue[key] = [value];
        return this.getItem(key)
            .then((data) => {
                try {
                    const modifiedData = this.applyMerge(key, data);

                    // Clean up the write queue so we
                    // don't apply these changes again
                    delete this.mergeQueue[key];
