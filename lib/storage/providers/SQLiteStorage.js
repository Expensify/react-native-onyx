...
const provider = {
    ...
    multiGet(keys) {
        return QuickSQLite.executeAsync(DB_NAME, `SELECT record_key, valueJSON from magic_map where record_key IN (${new Array(keys.length).fill('?').join(',')});`, keys)
            .then(({rows}) => {
                // eslint-disable-next-line no-underscore-dangle
                const res = _.map(rows._array, row => [row.record_key, JSON.parse(row.valueJSON)]);
                return res;
            });
    },
    ...
    /**
      ...
      */
    multiSet(pairs) {
        const stringifiedArgs = _.map(pairs, ele => [
            ele[0],
            JSON.stringify(ele[1]),
        ]);
        return QuickSQLite.executeBatchAsync(DB_NAME, [['REPLACE into magic_map (record_key, valueJSON) VALUES (?, json(?));', stringifiedArgs]]);
    },
    ...
    getAllKeys: () => QuickSQLite.executeAsync(DB_NAME, 'SELECT record_key from magic_map;').then(({rows}) => {
        // eslint-disable-next-line no-underscore-dangle
        const res = _.map(rows._array, row => row.record_key);
        return res;
    }),
