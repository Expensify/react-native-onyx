    ...
    */
    multiMerge(pairs) {
        ...
        _.forEach(pairs, (pair) => {
            ...
            queryArguments.push([pair[0], JSON.stringify(value)]);
        });

        const deletePromise = this.removeItems(keysToDelete);
        const mergePromise = queryArguments.length !== 0 ? db.executeBatchAsync([[query, queryArguments]]) : Promise.resolve();
