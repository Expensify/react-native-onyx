...
function get(key) {
    ...
    if (cache.hasCacheForKey(key)) {
        // TODO: fix this
        // const cachedValueWithoutNestedNulls = utils.removeNestedNullValues(cache.getValue(key));
...
function keysChanged(collectionKey, partialCollection, notifyRegularSubscibers = true, notifyWithOnyxSubscibers = true) {
    ...
    for (let i = 0; i < stateMappingKeys.length; i++) {
        ...
        const cachedCollectionWithoutNestedNulls = utils.removeNestedNullValues(cachedCollection);
        // const cachedCollectionWithoutNestedNulls = cachedCollection;
        ...
        if (_.isFunction(subscriber.callback)) {
            ...
            if (isSubscribedToCollectionMemberKey) {
                subscriber.callback(cachedCollectionWithoutNestedNulls[subscriber.key], subscriber.key);
                continue;
            }
            ...
            if (isSubscribedToCollectionKey) {
                ...
                if (subscriber.selector) {
                    subscriber.withOnyxInstance.setStateProxy((prevState) => {
                        ...
                        const newData = reduceCollectionWithSelector(cachedCollectionWithoutNestedNulls, subscriber.selector, subscriber.withOnyxInstance.state);
                ...
                subscriber.withOnyxInstance.setStateProxy((prevState) => {
                    ...
                    for (let j = 0; j < dataKeys.length; j++) {
                        const dataKey = dataKeys[j];
                        finalCollection[dataKey] = cachedCollectionWithoutNestedNulls[dataKey];
                    }
            ...
            if (isSubscribedToCollectionMemberKey) {
                ...
                if (subscriber.selector) {
                    subscriber.withOnyxInstance.setStateProxy((prevState) => {
                        ...
                        const newData = getSubsetOfData(cachedCollectionWithoutNestedNulls[subscriber.key], subscriber.selector, subscriber.withOnyxInstance.state);
 ...
 */
function removeNullValues(key, value, shouldRemoveNestedNullsInObjects = true) {
    if (_.isNull(value)) {
        remove(key);
    ...
    return {value: shouldRemoveNestedNullsInObjects ? utils.removeNestedNullValues(value) : value, wasRemoved: false};
}
 ...
 */
function prepareKeyValuePairsForStorage(data, shouldRemoveNestedNullsInObjects) {
    const keyValuePairs = [];

    _.forEach(data, (value, key) => {
        const {value: valueAfterRemoving, wasRemoved} = removeNullValues(key, value, shouldRemoveNestedNullsInObjects);
 ...
 * @returns {*}
 */
function applyMerge(existingValue, changes, shouldRemoveNestedNullsInObjects) {
    ...
    if (_.some(changes, _.isObject)) {
        ...
        return _.reduce(changes, (modifiedData, change) => utils.fastMerge(modifiedData, change, shouldRemoveNestedNullsInObjects), existingValue || {});
    }
