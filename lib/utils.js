...
function mergeObject(target, source, shouldRemoveNullObjectValues = true) {
    ...
    if (isMergeableObject(target)) {
        ...
        for (let i = 0; i < targetKeys.length; ++i) {
            ...
            if (shouldRemoveNullObjectValues && (target[key] == null || source[key] === null)) { continue; }
    ...
    for (let i = 0; i < sourceKeys.length; ++i) {
        ...
        // If shouldRemoveNullObjectValues is true, we want to remove null values from the merged object
        // eslint-disable-next-line no-continue
        if (source[key] === undefined || (shouldRemoveNullObjectValues && source[key] === null)) { continue; }

        if (!isMergeableObject(source[key]) || !target[key]) {
            // eslint-disable-next-line no-continue
            if (targetAndSourceIdentical) { continue; }
