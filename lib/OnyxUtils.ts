...
let callbackToStateMapping: Record<
  string,
  CallbackToStateMapping<OnyxKey>
> = {};
...
let lastConnectionCallbackData = new Map<
  number,
  { value: OnyxValue<OnyxKey>; matchedKey: OnyxKey | undefined }
>();
...
function initStoreValues(
  ...
  initialKeyStates: Partial<KeyValueMapping>,
  ...
  if (
    typeof keys.COLLECTION === "object" &&
    typeof keys.COLLECTION.SNAPSHOT === "string"
  ) {
...
function sendActionToDevTools(
  method:
    | typeof METHOD.MERGE_COLLECTION
    | typeof METHOD.MULTI_SET
    | typeof METHOD.SET_COLLECTION,
  key: undefined,
...
function sendActionToDevTools(
  ...
  DevTools.registerAction(
    utils.formatActionName(method, key),
    value,
    key
      ? { [key]: mergedValue !== undefined ? mergedValue : value }
      : (value as OnyxCollection<KeyValueMapping[OnyxKey]>),
  );
}
...
function reduceCollectionWithSelector<TKey extends CollectionKeyBase, TReturn>(
  ...
  return Object.entries(collection ?? {}).reduce(
    (finalCollection: Record<string, TReturn>, [key, item]) => {
...
function get<TKey extends OnyxKey, TValue extends OnyxValue<TKey>>(
  key: TKey,
): Promise<TValue> {
  ...
  const promise = Storage.getItem(key)
    ...
    .catch((err) =>
      Logger.logInfo(
        `Unable to get item from persistent storage. Key: ${key} Error: ${err}`,
      ),
    );
...
function multiGet<TKey extends OnyxKey>(
  keys: CollectionKeyBase[],
): Promise<Map<OnyxKey, OnyxValue<TKey>>> {
  ...
  for (const key of keys) {
    ...
    if (cache.hasPendingTask(pendingKey)) {
      pendingTasks.push(
        cache.getTaskPromise(pendingKey) as Promise<OnyxValue<TKey>>,
      );
          ...
          if (skippableCollectionMemberIDs.size) {
            try {
              const [, collectionMemberID] =
                OnyxKeys.splitCollectionMemberKey(key);
              if (skippableCollectionMemberIDs.has(collectionMemberID)) {
 ...
 */
function tupleGet<Keys extends readonly OnyxKey[]>(
  keys: Keys,
): Promise<{ [Index in keyof Keys]: OnyxValue<Keys[Index]> }> {
...
function deleteKeyBySubscriptions(subscriptionID: number) {
  ...
  if (subscriber && onyxKeyToSubscriptionIDs.has(subscriber.key)) {
    const updatedSubscriptionsIDs = onyxKeyToSubscriptionIDs
      .get(subscriber.key)
      .filter((id: number) => id !== subscriptionID);
 ...
 */
function tryGetCachedValue<TKey extends OnyxKey>(
  key: TKey,
): OnyxValue<OnyxKey> {
...
function getCachedCollection<TKey extends CollectionKeyBase>(
  collectionKey: TKey,
  collectionMemberKeys?: string[],
): NonNullable<OnyxCollection<KeyValueMapping[TKey]>> {
  ...
  if (
    ...
    (Array.isArray(allKeys) ? allKeys.length > 0 : allKeys.size > 0)
  ) {
  ...
  for (const key of allKeys) {
    ...
    if (
      ...
      !OnyxKeys.isCollectionMemberKey(collectionKey, key)
    ) {
      continue;
    }
...
function keysChanged<TKey extends CollectionKeyBase>(
  ...
  const collectionSubscriberIDs =
    onyxKeyToSubscriptionIDs.get(collectionKey) ?? [];
  ...
  for (const subID of collectionSubscriberIDs) {
    ...
    try {
      ...
      Logger.logAlert(
        `[OnyxUtils.keysChanged] Subscriber callback threw an error for key '${collectionKey}': ${formatCaughtError(error)}`,
      );
    }
  }
  ...
  for (const subID of memberSubscriberIDs) {
    ...
    if (
      cachedCollection[subscriber.key] === previousCollection[subscriber.key]
    ) {
      continue;
    }
    ...
    try {
      const subscriberCallback =
        subscriber.callback as DefaultConnectCallback<TKey>;
      subscriberCallback(
        cachedCollection[subscriber.key],
        subscriber.key as TKey,
      );
      lastConnectionCallbackData.set(subscriber.subscriptionID, {
...
function keyChanged<TKey extends OnyxKey>(
  ...
  canUpdateSubscriber: (
    subscriber?: CallbackToStateMapping<OnyxKey>,
  ) => boolean = () => true,
  ...
  const cachedCollections: Record<
    string,
    ReturnType<typeof getCachedCollection>
  > = {};
  ...
  for (const stateMappingKey of stateMappingKeys) {
    ...
    if (
      !subscriber ||
      !OnyxKeys.isKeyMatch(subscriber.key, key) ||
      !canUpdateSubscriber(subscriber)
    ) {
      continue;
    }
    ...
    if (typeof subscriber.callback === "function") {
      try {
        const lastData = lastConnectionCallbackData.get(
          subscriber.subscriptionID,
        );
        if (
          lastData &&
          lastData.matchedKey === key &&
        ...
        Logger.logAlert(
          `[OnyxUtils.keyChanged] Subscriber callback threw an error for key '${key}': ${formatCaughtError(error)}`,
        );
      }
    ...
    console.error(
      "Warning: Found a matching subscriber to a key that changed, but no callback could be found.",
    );
  }
}
 ...
 */
function sendDataToConnection<TKey extends OnyxKey>(
  mapping: CallbackToStateMapping<TKey>,
  ...
  if (
    lastData &&
    lastData.matchedKey === matchedKey &&
    shallowEqual(lastData.value, value)
  ) {
    return;
  }

  (mapping.callback as DefaultConnectCallback<TKey> | undefined)?.(
    value,
    matchedKey as TKey,
  );
}
 ...
 */
function getCollectionDataAndSendAsObject<TKey extends OnyxKey>(
  matchingKeys: CollectionKeyBase[],
  mapping: CallbackToStateMapping<TKey>,
): void {
 ...
 */
function remove<TKey extends OnyxKey>(
  key: TKey,
...
function reportStorageQuota(error?: Error): Promise<void> {
  return Storage.getDatabaseSize()
    ...
    .catch((dbSizeError) => {
      Logger.logAlert(
        `Unable to get database size. getDatabaseSize error: ${dbSizeError}. Original error: ${error}`,
      );
    });
}
...
function retryOperation<TMethod extends RetriableOnyxOperation>(
  ...
  if (
    errorClass === StorageErrorClass.CAPACITY &&
    !StorageCircuitBreaker.isAllowed()
  ) {
  ...
  if (errorClass === StorageErrorClass.INVALID_DATA) {
    Logger.logAlert(
      `Attempted to set invalid data set in Onyx. Please ensure all data is serializable. Error: ${error}`,
    );
    throw error;
  }
  ...
  if (
    ...
    errorClass === StorageErrorClass.FATAL
  ) {
    Logger.logInfo(
      `Storage operation skipped retry; ${errorClass} errors are handled by the connection layer. Error: ${error}. onyxMethod: ${onyxMethod.name}.`,
    );
    return Promise.resolve();
  }
  ...
  if (nextRetryAttempt > MAX_STORAGE_OPERATION_RETRY_ATTEMPTS) {
    Logger.logAlert(
      `Storage operation failed after ${MAX_STORAGE_OPERATION_RETRY_ATTEMPTS} retries. Error: ${error}. onyxMethod: ${onyxMethod.name}.`,
    );
    return Promise.resolve();
  }
  ...
  if (!keyForRemoval) {
    ...
    Logger.logAlert(
      `Out of storage. But found no acceptable keys to remove. Error: ${error}`,
    );
  ...
  Logger.logInfo(
    `Out of storage. Evicting least recently accessed key (${keyForRemoval}) and retrying. Error: ${error}`,
  );
 ...
 */
function broadcastUpdate<TKey extends OnyxKey>(
  key: TKey,
  value: OnyxValue<TKey>,
  hasChanged?: boolean,
): void {
  ...
  for (const [key, value] of Object.entries(data)) {
    ...
    const valueWithoutNestedNullValues =
      (shouldRemoveNestedNulls ?? true)
        ? utils.removeNestedNullValues(value)
        : value;

    if (valueWithoutNestedNullValues !== undefined) {
      pairs.push([
        key,
        valueWithoutNestedNullValues,
        replaceNullPatches?.[key],
      ]);
    }
  }
...
function mergeChanges<
  TValue extends OnyxInput<OnyxKey>,
  TChange extends OnyxInput<OnyxKey>,
>(changes: TChange[], existingValue?: TValue): FastMergeResult<TChange> {
...
>(
  ...
  if (changes.some((change) => change && typeof change === "object")) {
    ...
    return changes.reduce<FastMergeResult<TChange>>(
      (modifiedData, change) => {
        const options: FastMergeOptions =
          mode === "merge"
            ? { shouldRemoveNestedNulls: true, objectRemovalMode: "replace" }
            : { objectRemovalMode: "mark" };
        const { result, replaceNullPatches } = utils.fastMerge(
          modifiedData.result,
          change,
          options,
        );
        ...
        // eslint-disable-next-line no-param-reassign
        modifiedData.replaceNullPatches = [
          ...modifiedData.replaceNullPatches,
          ...replaceNullPatches,
        ];
  ...
  return Storage.getAll()
    .then((pairs) => {
      ...
      for (const [key, value] of pairs) {
        ...
        if (
          skippableCollectionMemberIDs.size &&
      ...
      const defaultKeysFromStorage = Object.keys(defaultKeyStates).reduce(
    ...
    .catch((error) => {
      Logger.logAlert(
        `Failed to load data from storage during init. The app will boot with default key states only. Error: ${error}`,
      );
 ...
 */
function isValidNonEmptyCollectionForMerge<TKey extends CollectionKeyBase>(
  collection: OnyxMergeCollectionInput<TKey>,
): boolean {
  return (
    typeof collection === "object" &&
    !Array.isArray(collection) &&
    !utils.isEmptyObject(collection)
  );
}
 ...
 */
function doAllCollectionItemsBelongToSameParent<TKey extends CollectionKeyBase>(
  collectionKey: TKey,
  ...
  for (const dataKey of collectionKeys) {
    ...
    if (process.env.NODE_ENV === "development") {
      throw new Error(
        `Provided collection doesn't have all its data belonging to the same parent. CollectionKey: ${collectionKey}, DataKey: ${dataKey}`,
      );
    }
 ...
 */
function subscribeToKey<TKey extends OnyxKey>(
  ...
  callbackToStateMapping[subscriptionID] =
    mapping as CallbackToStateMapping<OnyxKey>;
  ...
  storeKeyBySubscriptions(
    mapping.key,
    callbackToStateMapping[subscriptionID].subscriptionID,
  );
  ...
  deferredInitTask.promise
    ...
    .then(() => {
      ...
      if (
        ...
        !OnyxKeys.isCollectionKey(mapping.key) &&
        cache.getAllKeys().has(mapping.key)
      ) {
    ...
    .then((keys) => {
      ...
      if (matchingKeys.length === 0) {
        ...
        const matchedKey = OnyxKeys.isCollectionKey(mapping.key)
          ? mapping.key
          : undefined;
...
function updateSnapshots<TKey extends OnyxKey>(
  data: Array<OnyxUpdate<TKey>>,
  mergeFn: typeof Onyx.merge,
): Array<() => Promise<void>> {
  ...
  for (const [snapshotEntryKey, snapshotEntryValue] of Object.entries(
    snapshotCollection,
  )) {
    ...
    for (const { key, value } of data) {
      ...
      if (
        typeof snapshotEntryValue !== "object" ||
        !("data" in snapshotEntryValue)
      ) {
        continue;
      }
      ...
      const newValue =
        typeof value === "object" && value !== null
          ? utils.pick(value as Record<string, unknown>, [...keysToCopy])
          : {};
 ...
 */
function setWithRetry<TKey extends OnyxKey>(
  { key, value, options }: SetParams<TKey>,
  retryAttempt?: number,
): Promise<void> {
  ...
  const {
    ...
    existingValueType,
    newValueType,
    isEmptyArrayCoercion,
  } = utils.checkCompatibilityWithExistingValue(value, existingValue);
  if (isEmptyArrayCoercion) {
    ...
    Logger.logAlert(
      `[ENSURE_BUGBOT] Onyx setWithRetry called on key "${key}" whose existing value is an empty array. Will coerce to object.`,
    );
  }
  if (!isCompatible) {
    Logger.logAlert(
      logMessages.incompatibleUpdateAlert(
        key,
        "set",
        existingValueType,
        newValueType,
  ...
  const hasChanged = options?.skipCacheCheck
    ? true
    : cache.hasValueChanged(key, valueWithoutNestedNullValues);
  ...
  return Storage.setItem(key, valueWithoutNestedNullValues)
    ...
    .catch((error) =>
      OnyxUtils.retryOperation(
        error,
        setWithRetry,
        { key, value: valueWithoutNestedNullValues, options },
        retryAttempt,
      ),
    )
    .then(() => {
...
function multiSetWithRetry(
  ...
  if (skippableCollectionMemberIDs.size) {
    newData = Object.keys(newData).reduce((result: OnyxMultiSetInput, key) => {
      try {
        ...
        // eslint-disable-next-line no-param-reassign
        result[key] = !skippableCollectionMemberIDs.has(collectionMemberID)
          ? newData[key]
          : null;
      } catch {
  ...
  const keyValuePairsToSet = OnyxUtils.prepareKeyValuePairsForStorage(
    newData,
    true,
  );
 ...
 * @param retryAttempt retry attempt
 */
function setCollectionWithRetry<TKey extends CollectionKeyBase>(
  { collectionKey, collection }: SetCollectionParams<TKey>,
  retryAttempt?: number,
): Promise<void> {
  ...
  if (
    !OnyxUtils.doAllCollectionItemsBelongToSameParent(
      collectionKey,
      resultCollectionKeys,
    )
  ) {
    Logger.logAlert(
      `setCollection called with keys that do not belong to the same parent ${collectionKey}. Skipping this update.`,
    );
    return Promise.resolve();
  }

  if (skippableCollectionMemberIDs.size) {
    resultCollection = resultCollectionKeys.reduce(
      (result: OnyxInputKeyValueMapping, key) => {
        try {
          const [, collectionMemberID] = OnyxKeys.splitCollectionMemberKey(
            key,
            collectionKey,
          );
          // If the collection member key is a skippable one we set its value to null.
  ...
  return OnyxUtils.getAllKeys().then((persistedKeys) => {
    ...
    const keyValuePairs = OnyxUtils.prepareKeyValuePairsForStorage(
      mutableCollection,
      true,
      undefined,
      true,
...
function mergeCollectionWithPatches<TKey extends CollectionKeyBase>(
  {
    ...
    isProcessingCollectionUpdate = false,
  }: MergeCollectionWithPatchesParams<TKey>,
  retryAttempt?: number,
): Promise<void> {
  if (!isValidNonEmptyCollectionForMerge(collection)) {
    Logger.logInfo(
      "mergeCollection() called with invalid or empty value. Skipping this update.",
    );
    return Promise.resolve();
  }
  ...
  // Confirm all the collection keys belong to the same parent
  if (
    !doAllCollectionItemsBelongToSameParent(collectionKey, resultCollectionKeys)
  ) {
    return Promise.resolve();
  }
  ...
  return getAllKeys()
    .then((persistedKeys) => {
      ...
      const cachedCollectionForExistingKeys = getCachedCollection(
        collectionKey,
        existingKeys,
      );

      const existingKeyCollection = existingKeys.reduce(
        (obj: OnyxInputKeyValueMapping, key) => {
          const {
            isCompatible,
            existingValueType,
            newValueType,
            isEmptyArrayCoercion,
          } = utils.checkCompatibilityWithExistingValue(
            resultCollection[key],
          ...
          if (isEmptyArrayCoercion) {
            ...
            // looking at a bugbot from this message, we're probably missing that key in OnyxKeys::KEYS_REQUIRING_EMPTY_OBJECT
            Logger.logAlert(
              `[ENSURE_BUGBOT] Onyx mergeCollection called on key "${key}" whose existing value is an empty array. Will coerce to object.`,
            );
          }
          if (!isCompatible) {
            Logger.logAlert(
      ...
      const keyValuePairsForExistingCollection = prepareKeyValuePairsForStorage(
      ...
      const keyValuePairsForNewCollection = prepareKeyValuePairsForStorage(
        newCollection,
        true,
      );
      ...
      const prewarmPromise = hasColdExistingKey
        ? multiGet(existingKeys).catch((err) =>
            Logger.logInfo(
              `mergeCollectionWithPatches pre-warm failed; proceeding with cache-only merge. Error: ${err}`,
            ),
          )
        : Promise.resolve();
      return prewarmPromise.then(() => {
        ...
        // write fails.
        const previousCollection = getCachedCollection(
          collectionKey,
          existingKeys,
        );
        ...
        if (
          !OnyxKeys.isRamOnlyKey(collectionKey) &&
          keyValuePairsForExistingCollection.length > 0
        ) {
        ...
        if (
          !OnyxKeys.isRamOnlyKey(collectionKey) &&
          keyValuePairsForNewCollection.length > 0
        ) {
          promises.push(Storage.multiSet(keyValuePairsForNewCollection));
        }

        const inFlightKeys = new Set<OnyxKey>(
          Object.keys(finalMergedCollection),
        );
 ...
 * @param retryAttempt retry attempt
 */
function partialSetCollection<TKey extends CollectionKeyBase>(
  { collectionKey, collection }: SetCollectionParams<TKey>,
  retryAttempt?: number,
): Promise<void> {
  let resultCollection: OnyxInputKeyValueMapping = collection;
  ...
  return getAllKeys().then((persistedKeys) => {
    const mutableCollection: OnyxInputKeyValueMapping = { ...resultCollection };
    const existingKeys = resultCollectionKeys.filter((key) =>
      persistedKeys.has(key),
    );
    const previousCollection = getCachedCollection(collectionKey, existingKeys);
    const keyValuePairs = prepareKeyValuePairsForStorage(
      mutableCollection,
      true,
      undefined,
      true,
...
function logKeyChanged(
  onyxMethod: Extract<OnyxMethod, "set" | "merge">,
  key: OnyxKey,
  value: unknown,
  hasChanged: boolean,
) {
  Logger.logInfo(
    `${onyxMethod} called for key: ${key}${_.isObject(value) ? ` properties: ${_.keys(value).join(",")}` : ""} hasChanged: ${hasChanged}`,
  );
}

function logKeyRemoved(
  onyxMethod: Extract<OnyxMethod, "set" | "merge">,
  key: OnyxKey,
) {
  Logger.logInfo(
    `${onyxMethod} called for key: ${key} => null passed, so key was removed`,
  );
}
