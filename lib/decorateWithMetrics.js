...
class TaggedItem {
    /**
     * @param {Object} data - the original result of a decorated function
     * @param {String} [alias] - optionally an alias to identify what created the result
     */
    constructor(data, alias) {
        // Shallow clone the original data
        Object.assign(this, data);

        // Keep a ref to trace retainers of the original data
        this.ref = data;
        this.alias = alias;
    }
}
        ...
        * */
        Promise.resolve(originalReturnValue)
            .then((data) => {
                ...
                if (_.isObject(data)) {
                    const tag = new TaggedItem(data, alias);
