...
function get(key) {
    ...
    /* Otherwise setup a task to retrieve it.
       This ensures concurrent usages would not start the same task over and over */
    const getFromStorage = () => AsyncStorage.getItem(key)
        .then((val) => {
    ...
    return cache.resolveTask(`get:${key}`, getFromStorage);
}
...
function getAllKeys() {
    ...
    /* Otherwise setup a task to retrieve them.
       This ensures concurrent usages would not start the same task over and over */
    const getKeysFromStorage = () => AsyncStorage.getAllKeys()
        .then((keys) => {
    ...
    return cache.resolveTask('getAllKeys', getKeysFromStorage);
}
...
function cleanCache(key) {
    ...
    // When this is a collection - also recursively removed any unused individual items
