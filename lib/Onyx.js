...
function get(key) {
    ...
    const promise = MultiStorage.getItem(key)
        .then((val) => {
            cache.set(key, val);
            return val;
        })
...
function getAllKeys() {
    ...
    const promise = MultiStorage.getAllKeys().then((keys) => {
...
function remove(key) {
    ...
    return MultiStorage.removeItem(key);
}
...
function reportStorageQuota() {
    return MultiStorage.getDatabaseSize()
...
function set(key, value) {
    ...
    // If the value has not changed or the key got removed, calling MultiStorage.setItem() would be redundant and a waste of performance, so return early instead.
    if (!hasChanged || wasRemoved) {
        return updatePromise;
    }

    return MultiStorage.setItem(key, valueAfterRemoving)
        .catch((error) => {
            console.log('eviction', error);
            evictStorageAndRetry(error, set, key, valueAfterRemoving);
        })
        .then(() => {
            console.log('success');
            return updatePromise;
...
function multiSet(data) {
    ...
    return MultiStorage.multiSet(keyValuePairs)
...
function merge(key, changes) {
    ...
    mergeQueuePromise[key] = get(key).then((existingValue) => {
        ...
        try {
            ...
            // If the value has not changed, calling MultiStorage.setItem() would be redundant and a waste of performance, so return early instead.
            if (!hasChanged || isClearing || wasRemoved) {
                return updatePromise;
            }

            return MultiStorage.mergeItem(key, batchedChanges, modifiedData).then(() => updatePromise);
        } catch (error) {
...
function initializeWithDefaultKeyStates() {
    return MultiStorage.multiGet(_.keys(defaultKeyStates)).then((pairs) => {
 ...
 * key state may store an unexpected value in MultiStorage.
 *
 * E.g.
 * Onyx.clear();
 * Onyx.set(ONYXKEYS.DEFAULT_KEY, 'default');
 * MultiStorage.getItem(ONYXKEYS.DEFAULT_KEY)
 *     .then((storedValue) => console.log(storedValue));
 * null is logged instead of the expected 'default'
 *
 * Onyx.set() might call MultiStorage.setItem() before Onyx.clear() calls
 * MultiStorage.setItem(). Use Onyx.merge() instead if possible. Onyx.merge() calls
 * Onyx.get(key) before calling MultiStorage.setItem() via Onyx.set().
 * MultiStorage.setItem() from Onyx.clear() will have already finished and the merged
...
function clear(keysToPreserve = []) {
    ...
    return getAllKeys().then((keys) => {
        ...
        return MultiStorage.removeItems(keysToBeClearedFromStorage)
            .then(() => MultiStorage.multiSet(defaultKeyValuePairs))
            .then(() => {
                isClearing = false;
                Broadcast.sendMessage({type: METHOD.CLEAR, keysToPreserve});
...
function mergeCollection(collectionKey, collection) {
    ...
    return getAllKeys().then((persistedKeys) => {
        ...
        if (keyValuePairsForExistingCollection.length > 0) {
            promises.push(MultiStorage.multiMerge(keyValuePairsForExistingCollection));
        }

        if (keyValuePairsForNewCollection.length > 0) {
            promises.push(MultiStorage.multiSet(keyValuePairsForNewCollection));
        }
...
function setMemoryOnlyKeys(keyList) {
    MultiStorage.setMemoryOnlyKeys(keyList);
...
function init({
    ...
    if (shouldSyncMultipleInstances && _.isFunction(MultiStorage.keepInstancesSync)) {
        MultiStorage.keepInstancesSync((key, value) => {
            cache.set(key, value);
