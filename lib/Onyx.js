...
function clear(keysToPreserve = []) {
    return getAllKeys()
        .then((keys) => {
            ...
            // Extract all the keys and clear them out
            const keysToReset = _.map(keyValuesToReset, keyValueToReset => keyValueToReset[0]);
            return Storage.multiRemove(keysToReset);
        });
}
