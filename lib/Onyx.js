...
function keysChanged(collectionKey, partialCollection) {
    ...
    // and does not represent all of the combined keys and values for a collection key. It is just the "new" data that was qd in via mergeCollection().
...
function set(key, value) {
    return notifyAndWriteToStorage(key, value);
}
...
function merge(key, value) {
    ...
    return new Promise((resolve) => {
        get(key).then(currentValue => Storage.mergeItem(key, value, currentValue).then(updatedValue => resolve(notifyAndWriteToStorage(key, updatedValue))));
    });
}
