...
function sendDataToConnection(config, val, matchedKey) {
    ...
    if (config.withOnyxInstance) {
        ...
        if (config.selector) {
            const newValue = lodashGet(val, config.selector);
            PerformanceUtils.logSetStateCall(config, null, newValue, 'sendDataToConnection');
            config.withOnyxInstance.setWithOnyxState(config.statePropertyName, newValue);
            return;
        }

        if (config.reducer) {
            const newValue = config.reducer(val);
            PerformanceUtils.logSetStateCall(config, null, newValue, 'sendDataToConnection');
            config.withOnyxInstance.setWithOnyxState(config.statePropertyName, newValue);
            return;
        }
        ...
        if (config.selector) {
            config.callback(lodashGet(val, config.selector), matchedKey);
            return;
        }
        if (config.reducer) {
            config.callback(config.reducer(val), matchedKey);
            return;
        }
