 ...
 * @returns {Promise}
 */
function waitPendingWrites(nextModification, key) {
    // If key is not set, then we are doing a modification that affects all keys. We must wait for all prior modifications to complete.
    if (key == null) {
...
function remove(key) {
    ...
    return waitPendingWrites(nextModification, key);
}
...
function clear(keysToPreserve = []) {
    ...
    return waitPendingWrites(nextModification);
}
