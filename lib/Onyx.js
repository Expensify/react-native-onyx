 ...
 * @param {*} previousData
 */
function keyChanged(key, data, previousData) {
    ...
    for (let i = stateMappingKeys.length; i--;) {
        ...
        if (_.isFunction(subscriber.callback)) {
            ...
            if (subscriber.selector) {
                const previousValue = lodashGet(previousData, subscriber.selector);
...
function sendDataToConnection(config, val, matchedKey) {
    ...
    if (config.withOnyxInstance) {
        ...
        if (config.selector) {
            config.withOnyxInstance.setWithOnyxState(config.statePropertyName, lodashGet(val, config.selector));
            return;
        }
...
function notifySubscribersOnNextTick(key, value, previousValue) {
    Promise.resolve().then(() => keyChanged(key, value, previousValue));
}
...
function set(key, value) {
    ...
    cache.set(key, value);
    notifySubscribersOnNextTick(key, value, previousValue);
