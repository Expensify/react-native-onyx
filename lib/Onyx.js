 ...
 * @returns {Object}
 */
const reduceCollectionWithSelector = (
    collection,
    selector,
    ...
    _.reduce(
        ...
        (finalCollection, item, key) => {
            ...
            finalCollection[key] = getSubsetOfData(
                item,
                selector,
                withOnyxInstanceState
            );
...
function isCollectionMemberKey(collectionKey, key) {
    return (
        Str.startsWith(key, collectionKey) && key.length > collectionKey.length
    );
}
...
function removeFromEvictionBlockList(key, connectionID) {
    evictionBlocklist[key] = _.without(
...
function getCachedCollection(collectionKey) {
    const collectionMemberKeys = _.filter(cache.getAllKeys(), (storedKey) =>
        isCollectionMemberKey(collectionKey, storedKey)
    );
...
function keysChanged(collectionKey, partialCollection) {
    ...
    for (let i = 0; i < stateMappingKeys.length; i++) {
         ...
         */
        const isSubscribedToCollectionMemberKey = isCollectionMemberKey(
            collectionKey,
            subscriber.key
        );
...
function keyChanged(key, data, canUpdateSubscriber) {
    ...
    for (let i = 0; i < stateMappingKeys.length; i++) {
        ...
        if (
            !subscriber ||
            !isKeyMatch(subscriber.key, key) ||
            (_.isFunction(canUpdateSubscriber) &&
        ...
        if (_.isFunction(subscriber.callback)) {
            if (
                ...
                subscriber.waitForCollectionCallback
            ) {
        ...
        console.error(
            "Warning: Found a matching subscriber to a key that changed, but no callback or withOnyxInstance could be found."
        );
    }
}
...
function sendDataToConnection(mapping, val, matchedKey) {
    ...
    if (mapping.withOnyxInstance) {
        ...
        if (mapping.selector) {
            if (isCollectionKey(mapping.key)) {
                newData = reduceCollectionWithSelector(
                    val,
                    mapping.selector,
                    mapping.withOnyxInstance.state
                );
            } else {
                newData = getSubsetOfData(
                    val,
                    mapping.selector,
                    mapping.withOnyxInstance.state
                );
        ...
        mapping.withOnyxInstance.setWithOnyxState(
...
function connect(mapping) {
    ...
    deferredInitTask.promise
        ...
        .then((keys) => {
            ...
            const matchingKeys = _.filter(keys, (key) =>
            ...
            if (_.isFunction(mapping.callback)) {
                if (isCollectionKey(mapping.key)) {
                    ...
                    for (let i = 0; i < matchingKeys.length; i++) {
                        get(matchingKeys[i]).then((val) =>
                            sendDataToConnection(mapping, val, matchingKeys[i])
                        );
                    }
                    return;
                }
                ...
                get(mapping.key).then((val) =>
                    sendDataToConnection(mapping, val, mapping.key)
                );
                return;
            }
            ...
            console.error(
                "Warning: Onyx.connect() was found without a callback or withOnyxInstance"
            );
        });
...
function disconnect(connectionID, keyToRemoveFromEvictionBlocklist) {
    ...
    if (keyToRemoveFromEvictionBlocklist) {
        removeFromEvictionBlockList(
            keyToRemoveFromEvictionBlocklist,
            connectionID
        );
    }
...
function evictStorageAndRetry(error, onyxMethod, ...args) {
    ...
    if (
        ...
        Logger.logAlert(
            "Attempted to set invalid data set in Onyx. Please ensure all data is serializable."
        );
        throw error;
    }
    ...
    const keyForRemoval = _.find(
        ...
        (key) => !evictionBlocklist[key]
    );

    if (!keyForRemoval) {
        Logger.logAlert(
            "Out of storage. But found no acceptable keys to remove."
        );
        throw error;
    }
    ...
    Logger.logInfo(
        `Out of storage. Evicting least recently accessed key (${keyForRemoval}) and retrying.`
    );
...
function broadcastUpdate(key, value, method) {
    ...
    Logger.logInfo(
        `${method}() called for key: ${key}${
            _.isObject(value) ? ` properties: ${_.keys(value).join(",")}` : ""
        }`
    );
    ...
    cache.set(key, value);
    notifySubscribersOnNextTick(
        key,
        value,
        (subscriber) => hasChanged || subscriber.initWithStoredValues === false
    );
}
...
function set(key, value) {
    ...
    return Storage.setItem(key, value).catch((error) =>
        evictStorageAndRetry(error, set, key, value)
    );
}
...
function multiSet(data) {
    ...
    return Storage.multiSet(keyValuePairs).catch((error) =>
        evictStorageAndRetry(error, multiSet, data)
    );
}
...
function merge(key, changes) {
    ...
    return get(key).then((existingValue) => {
        try {
            ...
            Logger.logAlert(
                `An error occurred while applying merge for key: ${key}, Error: ${error}`
            );
        }
...
function clear(keysToPreserve = []) {
    return getAllKeys().then((keys) => {
        ...
        _.each(keys, (key) => {
            ...
            if (!isKeyToPreserve) {
                ...
                if (newValue !== oldValue) {
                    ...
                    const collectionKey = key.substring(
                        0,
                        key.indexOf("_") + 1
                    );
                    if (collectionKey) {
        ...
        const defaultKeyValuePairs = _.pairs(
            _.omit(defaultKeyStates, keysToPreserve)
        );
        ...
        return Storage.removeItems(keysToBeClearedFromStorage).then(() =>
            Storage.multiSet(defaultKeyValuePairs)
        );
    });
}
...
function mergeCollection(collectionKey, collection) {
    ...
    return getAllKeys().then((persistedKeys) => {
        ...
        const keyValuePairsForExistingCollection =
            prepareKeyValuePairsForStorage(existingKeyCollection);
        const keyValuePairsForNewCollection =
            prepareKeyValuePairsForStorage(newCollection);
        ...
        if (keyValuePairsForExistingCollection.length > 0) {
            promises.push(
                Storage.multiMerge(keyValuePairsForExistingCollection)
            );
        }
        ...
        return Promise.all(promises).catch((error) =>
            evictStorageAndRetry(error, mergeCollection, collection)
        );
    });
}
...
function update(data) {
    ...
    _.each(data, ({ onyxMethod, key }) => {
        ...
        if (onyxMethod !== METHOD.CLEAR && !_.isString(key)) {
            throw new Error(
                `Invalid ${typeof key} key provided in Onyx update. Onyx key must be of type string.`
            );
        }
    });
...
function init({
    ...
    if (
        ...
        _.isFunction(Storage.keepInstancesSync)
    ) {
...
function applyDecorators() {
    ...
    mergeCollection = decorate.decorateWithMetrics(
        mergeCollection,
        "Onyx:mergeCollection"
    );
    getAllKeys = decorate.decorateWithMetrics(getAllKeys, "Onyx:getAllKeys");
    initializeWithDefaultKeyStates = decorate.decorateWithMetrics(
        initializeWithDefaultKeyStates,
        "Onyx:defaults"
    );
