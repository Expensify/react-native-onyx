...
function applyMerge(existingValue, changes) {
    ...
    if (_.isObject(lastChange)) {
        // Object values are merged one after the other
        // If there are non-object values in the changes, they will be overwritten in the end,
        // because the last value has to be an object.
        // fastMerge will just overwrite the existingValue with non-object values
        // ====================================================
    ...
    return get(key)
        .then((existingValue) => {
            try {
                ...
                // If the mergeQueue contains nullish values, it means that the user
                // intends to reset the existing value (undefined) or remove the key (null)
                // If there are other changes queued afterwards, we cannot simply batch the remaining changes
                // and merge it with the existingValue. Instead, in order for the nullish value to have an effect,
                // we have to omit the existingValue and replace it with the batchedChanges after the last nullish value.
