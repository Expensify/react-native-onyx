...
function clear(keysToPreserve = []) {
    return getAllKeys()
        .then((keys) => {
            ...
            // Make sure that the default key/values and the key/values to preserve
            // are never cleared and always kept in storage. This makes sure to always leave storage in a state that contains
            // all the default values and any additional values that we want to remain after the database is cleared.

            const retainedKeyValuePairs = [...defaultKeyValuePairs, ...keyValuesToPreserve];

            return Storage.getAllKeys().then((allKeys) => {
                const retainedKeys = _.map(retainedKeyValuePairs, ([key]) => key);
                const keysToBeCleared = _.difference(allKeys, retainedKeys);
                return Promise.all(_.map(keysToBeCleared, key => Storage.removeItem(key)))
                    .then(() => Storage.multiSet(retainedKeyValuePairs));
            });
        });
}
