 ...
 * @param {String} key
 * @param {*} data
 * @param {Boolean} onlyUpdateSubscribersWithoutInitialValues - When true we will only update the subscribers that have elected to initialize without the values from storage.
 * This is used when we skip updates to subscribers because the values in the cache have not changed. We don't want to skip updates to the subscribers that did not get the
 * data when they connected or else they will only get new data when the value changes.
 */
function keyChanged(key, data, onlyUpdateSubscribersWithoutInitialValues = false) {
    ...
    for (let i = stateMappingKeys.length; i--;) {
        const subscriber = callbackToStateMapping[stateMappingKeys[i]];
        if (!subscriber || !isKeyMatch(subscriber.key, key) || (onlyUpdateSubscribersWithoutInitialValues && subscriber.initWithStoredValues !== false)) {
            continue;
        }
 ...
 * @param {String} key
 * @param {*} value
 * @param {Boolean} [onlyUpdateInitWithStoredValuesFalse]
 */
// eslint-disable-next-line rulesdir/no-negated-variables
function notifySubscribersOnNextTick(key, value, onlyUpdateInitWithStoredValuesFalse = false) {
    Promise.resolve().then(() => keyChanged(key, value, onlyUpdateInitWithStoredValuesFalse));
}
...
function set(key, value) {
    ...
    if (!cache.hasValueChanged(key, value)) {
        notifySubscribersOnNextTick(key, value, true);
        return;
    }
