 ...
 * @returns {Promise}
 */
function set(key, value, resolveAfterBroadcast) {
    if (_.isNull(value)) {
        return remove(key);
    }
    ...
    const storagePromise = Storage.setItem(key, valueWithNullRemoved)
        .catch(error => evictStorageAndRetry(error, set, key, valueWithNullRemoved))
        .then(() => updatePromise);

    if (resolveAfterBroadcast) {
 ...
 * @returns {Promise}
 */
function multiSet(data, resolveAfterBroadcast) {
    ...
    const storagePromise = Storage.multiSet(keyValuePairs)
 ...
 * @returns {Promise}
 */
function merge(key, changes, resolveAfterBroadcast) {
    ...
    mergeQueuePromise[key] = get(key)
        .then((existingValue) => {
            try {
                ...
                const storagePromise = Storage.mergeItem(key, batchedChanges, modifiedData).then(() => updatePromise);
 ...
 * @returns {Promise}
 */
function mergeCollection(collectionKey, collection, resolveAfterBroadcast) {
    ...
    return getAllKeys()
        .then((persistedKeys) => {
            ...
            const storagePromise = Promise.all(promises)
...
function update(data) {
    ...
    _.each(data, ({onyxMethod, key, value}) => {
        switch (onyxMethod) {
            case METHOD.SET:
                promises.push(() => set(key, value, true));
                break;
            case METHOD.MERGE:
                promises.push(() => merge(key, value, true));
                break;
            case METHOD.MERGE_COLLECTION:
                promises.push(() => mergeCollection(key, value, true));
                break;
            case METHOD.MULTI_SET:
                promises.push(() => multiSet(value, true));
                break;
            case METHOD.CLEAR:
                clearPromise = clear();
                break;
    ...
    return clearPromise.then(() => promises.reduce((chain, p) => chain.then(p()), Promise.resolve()));
}
