...
function update(data) {
    ...
    _.each(data, ({onyxMethod, key, value}) => {
        switch (onyxMethod) {
            case METHOD.SET:
                updateQueue[key] = [{onyxMethod, value}];
                break;
            case METHOD.MERGE: {
                const operation = {onyxMethod, value};
                if (!updateQueue[key]) {
            ...
            case METHOD.MERGE_COLLECTION:
                _.each(value, (_data, dataKey) => {
                    const operation = {onyxMethod: METHOD.MERGE, value: _data};
                    if (!updateQueue[dataKey]) {
                        updateQueue[dataKey] = [operation];
                    } else {
                        updateQueue[dataKey].push(operation);
                    }
                });
                break;
            case METHOD.MULTI_SET:
                _.each(value, (_data, dataKey) => {
                    updateQueue[dataKey] = [{onyxMethod: METHOD.SET, value: _data}];
                });
                break;
            case METHOD.CLEAR:
                clearPromise = clear();
