 ...
 * @param {*} [partialValue] partial object value if we are calling set via Onyx.merge() and Storage.mergeItem() is supported.
 * @param {Boolean} [shouldMerge]
 * @returns {Promise}
 */
function write(key, value, partialValue, shouldMerge) {
    ...
    // Note: Storage.mergeItem() is only implemented in react-native-quick-sqlite for the time being.
    // There is no clear way to "merge" items with IndexedDB so the LocalForage provider does not have this method
    // and we will fallback to setting entire object values.
    if (partialValue && shouldMerge && Storage.mergeItem) {
        return Storage.mergeItem(key, partialValue)
            .catch(error => evictStorageAndRetry(error, write, key, value, partialValue, shouldMerge));
    }

    return Storage.setItem(key, value)
        .catch(error => evictStorageAndRetry(error, write, key, value));
}
 ...
 *
 * Only type `Object` should be merged. To set another type of value use `Onyx.set()`. Merge behavior uses the custom
 * merge function fastMerge() which will overwrite any array values of objects with new values.
 *
 * Calls to `Onyx.merge()` are batched so that any calls performed in a single tick will stack in a queue and get
 * applied in the order they were called.
 *
 * @example
        ...
        .then((data) => {
            try {
                ...
                if (_.isObject(data) || _.every(mergeValues, _.isObject)) {
                    partialData = _.reduce(mergeValues, (partialValues, mergeValue) =>
                        // eslint-disable-next-line prefer-object-spread, rulesdir/prefer-underscore-method, implicit-arrow-linebreak
                        Object.assign({}, fastMerge(partialValues, mergeValue)),
                    {});

                    // Object values are merged one after the other
                    newData = _.reduce(mergeValues, (modifiedData, mergeValue) => {
                        // lodash adds a small overhead so we don't use it here
                        // eslint-disable-next-line prefer-object-spread, rulesdir/prefer-underscore-method
                        const merged = Object.assign({}, fastMerge(modifiedData, mergeValue));

                        // We will also delete any object keys that are undefined or null.
                        // Deleting keys is not supported by AsyncStorage so we do it this way.
                        // Remove all first level keys that are explicitly set to null.
                        return _.omit(merged, (_value, finalObjectKey) => _.isNull(mergeValue[finalObjectKey]));
                    }, data || {});
                } else {
                    // If we have anything else we can't merge it so we'll simply return the last value that was queued
                    newData = _.last(mergeValues);
                }

                // Clean up the write queue so we don't apply these changes again
                delete mergeQueue[key];

                return set(key, newData, partialData, true);
            } catch (error) {
