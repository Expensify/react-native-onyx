...
import {logAlert, logInfo, registerLogger} from './Logger';
import OnyxInternal from './Onyx.internal';
...
function addAllSafeEvictionKeysToRecentlyAccessedList() {
    return OnyxInternal.getAllKeys()
        .then((keys) => {
        ...
        .then((keys) => {
            ...
            if (mapping.withOnyxInstance && isCollectionKey(mapping.key)) {
                ...
                _.each(matchingKeys, (key) => {
                    OnyxInternal.get(key).then(val => sendDataToConnection(mapping, val, key));
                });
            }
        });
...
function set(key, val) {
    ...
    return OnyxInternal.set(key, val)
...
// The code bellow is only bundled and applied when the ENV variable is enabled for a build
if (process.env.BENCHMARK_ONYX) {
    const decorate = require('./decorateWithMetrics');

    // decorate internal methods
    const privateMethods = ['get', 'getAllKeys'];
    decorate.decorateWithMetricsMultiple(OnyxInternal, privateMethods);

    // Decorate promise returning methods
    const publicMethods = [
        'addAllSafeEvictionKeysToRecentlyAccessedList',
        'multiSet',
        'clear',
    ...
    decorate.decorateWithMetricsMultiple(Onyx, publicMethods);
