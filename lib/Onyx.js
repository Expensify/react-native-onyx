...
// Let the app know when the storage has been exceeded
// eslint-disable-next-line rulesdir/no-negated-variables
let notifyClientOfMaxStorageReached = () => {};
...
function evictStorageAndRetry(error, onyxMethod, ...args) {
    ...
    if (!keyForRemoval) {
        ...
        // then we should stop retrying as there is not much the user can do to fix this. Instead of getting them stuck in an infinite loop we will
        // put them into memory-only mode and stop trying to store any new data at all.
        Logger.logAlert('Out of storage. But found no acceptable keys to remove.');

        // Client can provide a onMaxStorageReached event handler that we will call when exceeding the storage.
        notifyClientOfMaxStorageReached();
        Storage.disableWrites();
...
function set(key, value) {
    ...
    return Storage.setItem(key, valueWithNullRemoved)
        .catch(error => {
            ...
            evictStorageAndRetry(error, set, key, valueWithNullRemoved)
        });
}
...
function init({
    ...
    notifyClientOfMaxStorageReached = onMaxStorageReached;
