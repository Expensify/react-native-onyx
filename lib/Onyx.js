...
const reduceCollectionWithSelector = (collection, selector) => _.reduce(collection, (finalCollection, item, key) => {
    const selectedValue = getSubsetOfData(item, selector);

    // A selector is used to return a subset of a larger dataset.
    // If the selector didn't return any value, then nothing is added to the finalCollection.
    if (_.isUndefined(selectedValue)) {
        return finalCollection;
    }

    // eslint-disable-next-line no-param-reassign
    finalCollection[key] = selectedValue;
    return finalCollection;
}, {});
        ...
        if (_.isFunction(subscriber.callback)) {
            ...
            if (isSubscribedToCollectionKey) {
                ...
                for (let j = 0; j < dataKeys.length; j++) {
                    ...
                    // If the subscriber has a selector, then the consumer of this data must only be given the data
                    // returned by the selector and it should only be called when the selected data has changed
                    if (subscriber.selector) {
                        const newValue = getSubsetOfData(newData, subscriber.selector);
            ...
            if (isSubscribedToCollectionMemberKey) {
                ...
                if (subscriber.selector) {
                    const newValue = getSubsetOfData(cachedCollection[subscriber.key], subscriber.selector);
                    if (!_.isEqual(subscriber.lastCalledWith, newValue)) {
...
function keyChanged(key, data, canUpdateSubscriber) {
    ...
    for (let i = 0; i < stateMappingKeys.length; i++) {
        ...
        if (subscriber.withOnyxInstance) {
            ...
            // If the subscriber has a selector, then the component's state must only be updated the data
...
function sendDataToConnection(mapping, val, matchedKey) {
    ...
    if (mapping.withOnyxInstance) {
        ...
        if (mapping.selector) {
            ...
            const newValue = getSubsetOfData(val, mapping.selector);
