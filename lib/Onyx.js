...
function keyChanged(key, data, prevData, canUpdateSubscriber = () => true, notifyRegularSubscibers = true, notifyWithOnyxSubscibers = true) {
    ...
    for (let i = 0; i < stateMappingKeys.length; i++) {
        ...
        if (subscriber.withOnyxInstance) {
            ...
            if (subscriber.selector) {
                subscriber.withOnyxInstance.setStateProxy((prevState) => {
                    if (subscriber.key === 'exitSurveyReasonForm') {
                        console.log('RORY_DEBUG calling getSubsetOfData twice, with', prevState[subscriber.statePropertyName], data);
                        console.log('RORY_DEBUG prevState', prevState);
                        console.log('RORY_DEBUG subscriber.statePropertyName', subscriber.statePropertyName);
                    }
                    const previousValue = getSubsetOfData(_.pick(prevState, subscriber.statePropertyName), subscriber.selector, subscriber.withOnyxInstance.state);
                    const newValue = getSubsetOfData(data, subscriber.selector, subscriber.withOnyxInstance.state);

                    if (subscriber.key === 'exitSurveyReasonForm') {
                        console.log('RORY_DEBUG got shit from getSubsetOfData:', previousValue, newValue);
                    }
    ...
    if (mapping.withOnyxInstance) {
        ...
        if (mapping.selector) {
            if (isCollectionKey(mapping.key)) {
                ...
                if (mapping.key === 'exitSurveyReasonForm') {
                    console.log('RORY_DEBUG calling getSubsetOfData with sourceData:', val);
                    console.trace();
                }
 ...
 */
function remove(key) {
    const prevValue = cache.getValue(key);
    cache.drop(key);
    scheduleSubscriberUpdate(key, null, prevValue, 'RORY_DEBUG');
...
function clear(keysToPreserve = []) {
    return getAllKeys().then((keys) => {
        ...
        _.each(keyValuesToResetIndividually, (value, key) => {
            updatePromises.push(scheduleSubscriberUpdate(key, value, cache.getValue(key)));
        });
