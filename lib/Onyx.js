...
function merge(key, value) {
    // Array values are no longer concatenated when using Onyx.merge(). Our SQLite implementation in the native layer uses the MergePatch algorithm for JSON_PATCH
    // and will overwrite any arrays by default anyway so set() and merge() are effectively the same.
    if (_.isArray(value)) {
