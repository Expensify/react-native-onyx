...
/**
 * @typedef {(null|undefined|string|boolean|object|number)} JSONValue
 */
 ...
 * @param {string} key ONYXKEY to subscribe to
 * @param {number} connectionID
 * @param {string} [statePropertyName] the name of the property in the state to connect the data to
 * @param {object} [withOnyxInstance] whose setState() method will be called with any changed data
 *      This is used by React components to connect to Onyx
 * @param {function} [callback] a method that will be called with changed data
 *      This is used by any non-React code to connect to Onyx
 * @param {boolean} [initWithStoredValues] If set to false, then no data will be prefilled into the
 *  component
 * @param {boolean} [waitForCollectionCallback] If set to true, it will return the entire collection to the callback as a single object
 * @param {(string|function)} [selector] THIS PARAM IS ONLY USED WITH withOnyx(). If included, this will be used to subscribe to a subset of an Onyx key's data.
 ...
 * });
 *
 * @param {Mapping} mapping the mapping information to connect Onyx to the components state
 ...
 * @param {JSONValue} value value to store
 *
 * @returns {Promise}
 */
...
/**
 * @typedef {Array<Array<string,JSONValue>>} KeyValuePairArray
 */
/**
 ...
 *
 * @param {string[]} keysToPreserve is a list of ONYXKEYS that should not be cleared with the rest of the data
 * @returns {Promise}
 */
...
/**
 * @typedef {object} OnyxUpdate
 * @param {'set'|'merge'|'mergecollection'|'clear'} onyxMethod
 * @param {string} key
 * @param {JSONValue} value
 */
...
/**
 * @typedef {object} OnyxConfig
 * @param {object} [keys={}] `ONYXKEYS` constants object
 * @param {object} [initialKeyStates={}] initial data to set when `init()` and `clear()` is called
 * @param {string[]} [safeEvictionKeys=[]] This is an array of keys (individual or collection patterns) that when provided to Onyx are flagged
 ...
 * @param {number} [maxCachedKeysCount=55] Sets how many recent keys should we try to keep in cache Setting this to 0 would practically
 * mean no cache We try to free cache when we connect to a safe eviction key
 * @param {boolean} [captureMetrics=false] Enables Onyx benchmarking and exposes the get/print/reset functions
 * @param {boolean} [shouldSyncMultipleInstances] Auto synchronize storage events between multiple instances of Onyx running in different
 * tabs/windows. Defaults to true for platforms that support local storage (web/desktop)
 * @param {boolean} [debugSetState=false] Enables debugging setState() calls to connected components.
 */

/**
 * Initialize the store with actions and listening for storage events
 *
 * @param {OnyxConfig} [options={}] config object
 * @example
 * Onyx.init({
 *     keys: ONYXKEYS,
 *     initialKeyStates: {
