...
function applyMerge(existingValue, changes) {
    ...
    if (_.some(changes, _.isObject)) {
        // There might be undefined values in a batch of changes that we want to ignore when merging objects,
        // because otherwise they overwrite the previous changes in the batch.
        // Values of null will still overwrite the existing value
        // Therefore, we filter out undefined values before merging.
        const changesWithoutUndefined = _.filter(changes, change => !_.isUndefined(change));

        // Object values are then merged one after the other
        return _.reduce(changesWithoutUndefined, (modifiedData, change) => utils.fastMerge(modifiedData, change),
            existingValue || {});
    }
    ...
    mergeQueuePromise[key] = get(key)
        .then((existingValue) => {
            try {
                ...
                // If the mergeQueue included a null value, we want to overwrite the existing value
                // In this case, we can't simply merge the batched changes with the existing value, because then the null in the merge queue would have no effect
                const overwriteExistingValue = _.includes(mergeQueue[key], null);
                ...
                const modifiedData = utils.removeNullObjectValues(applyMerge(overwriteExistingValue ? undefined : existingValue, [batchedChanges]));
