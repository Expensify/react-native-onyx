...
function update(data) {
    ...
    return clearPromise.then(() =>
        multiGet(_.keys(updateQueue)).then((pairs) => {
            ...
            return Storage.multiSet(pairsForStorage).then(() => Promise.all(updatesToBroadcast)).then(() => {
                console.log(`Onyx: update complete in ${Math.trunc(performance.now() - start)}ms`);
            });
        }),
    );
}
