 ...
 */
function keysChanged(collectionKey, partialCollection, canUpdateSubscriber) {
    ...
    for (let i = 0; i < stateMappingKeys.length; i++) {
        ...
        if (!subscriber || (_.isFunction(canUpdateSubscriber) && !canUpdateSubscriber(subscriber))) {
            continue;
        }
        ...
        if (subscriber.withOnyxInstance) {
            ...
            if (isSubscribedToCollectionKey) {
                ...
                subscriber.withOnyxInstance.setState((prevState) => {
                    ...
                    // Make sure to return a copy of the data so that we are not able to mutate the collection keys in the cache
                    const allKeys = _.keys(finalCollection);
                    for (let j = 0; j < allKeys.length; j++) {
                        const dataKey = allKeys[j];
                        finalCollection[dataKey] = _.clone(cachedCollection[dataKey]);
                    }
...
/**
 * Create shallow copy of any object or array values
 *
 * @param {*} value
 * @returns {*}
 */
...
function set(key, value) {
    ...
    // If we have array or object values being set we will ensure they are immutable and have a different reference than before.
    const newValue = cloneValue(value);

    // If the value in the cache is the same value as what we have then do not update subscribers unless they
    // have initWithStoredValues: false then they MUST get all updates even if nothing has changed.
    if (!cache.hasValueChanged(key, newValue)) {
        cache.addToAccessedKeys(key);
        notifySubscribersOnNextTick(key, newValue, subscriber => subscriber.initWithStoredValues === false);
    ...
    // Always make sure that the value passed to the component or subscriber is different from the one in cache accidental mutation of the cached values is not possible
    notifySubscribersOnNextTick(key, cloneValue(newValue));

    // Write the thing to persistent storage, which will trigger a storage event for any other tabs open on this domain
    return Storage.setItem(key, newValue)
        .catch(error => evictStorageAndRetry(error, set, key, newValue));
}
 ...
 * @param {Object} partialCollection Object collection keyed by individual collection member keys and values. Note: This is not necessarily a complete collection
 *                                   and could be an incomplete set of of collection members.
 * @returns {Promise}
 */
function mergeCollection(collectionKey, partialCollection) {
    if (!_.isObject(partialCollection) || _.isArray(partialCollection) || _.isEmpty(partialCollection)) {
        Logger.logInfo('mergeCollection() called with invalid or empty value. Skipping this update.');
        return Promise.resolve();
    }

    // Confirm all the collection keys belong to the same parent
    _.each(partialCollection, (_data, dataKey) => {
    ...
    return getAllKeys()
        .then((persistedKeys) => {
            ...
            const [existingKeys, newKeys] = _.chain(partialCollection)
                .keys()
                .partition(key => persistedKeys.includes(key))
                .value();

            const existingKeyCollection = _.pick(partialCollection, existingKeys);
            const newCollection = _.pick(partialCollection, newKeys);
            ...
            Promise.all(_.map(existingKeys, get)).then(() => {
                console.log('partialCollection', partialCollection);

                // Check to see if the existing collection has changed from what we already have in the cache
                if (!cache.hasCollectionValueChanged(partialCollection)) {
                    console.log('collection value has not changed');
                    keysChanged(collectionKey, partialCollection, subscriber => subscriber.initWithStoredValues === false);
                    return;
                }

                console.log('collection value has changed');
                cache.merge(partialCollection);
                keysChanged(collectionKey, partialCollection);
            });

            return Promise.all(promises)
                .catch(error => evictStorageAndRetry(error, mergeCollection, partialCollection));
        });
}
