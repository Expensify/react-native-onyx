...
function set(key, value) {
    ...
    return Storage.setItem(key, value).catch(error => evictStorageAndRetry(error, set, key, value));
}
...
function multiSet(data) {
    ...
    return Storage.multiSet(keyValuePairs).catch(error => evictStorageAndRetry(error, multiSet, data));
}
...
function mergeCollection(collectionKey, collection) {
    ...
    return getAllKeys().then((persistedKeys) => {
        ...
        return Promise.all(promises).catch(error => evictStorageAndRetry(error, mergeCollection, collection));
    });
}
