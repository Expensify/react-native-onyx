...
function update(data) {
    ...
    return clearPromise.then(() =>
        Storage.multiGet(_.keys(updateQueue)).then((pairs) => {
            ...
            console.log('Onyx: updatedSnapshot pre-removal', updatedSnapshot);
            const updatedCollections = {};
            onyxCollectionKeySet.forEach((collectionKey) => {
                const updatedCollection = _.reduce(_.keys(updatedSnapshot), (acc, key) => {
                ...
                updatesToBroadcast.push(Promise.resolve().then(() => {
                    cache.merge(updatedCollection);
                    sendActionToDevTools(METHOD.MERGE_COLLECTION, collectionKey, updatedCollection, updatedCollection);
                    return keysChanged(collectionKey, updatedCollection);
                }));
            });
            console.log('Onyx: collections', updatedCollections);
            console.log('Onyx: individual keys', updatedSnapshot);

            _.mapObject(updatedSnapshot, (value, key) => {
                updatesToBroadcast.push(Promise.resolve().then(() => {
                    cache.set(key, value);
                    sendActionToDevTools(METHOD.MERGE, key, value, value);
                    return keyChanged(key, value, existingSnapshot[key], () => true);
                }));
            });
