 ...
 * @returns {Boolean}
 */
function verifyCollection(collectionKey, collection, type = 'merge') {
    if (!_.isObject(collection) || _.isArray(collection) || _.isEmpty(collection)) {
        Logger.logInfo(`${type}Collection() called with invalid or empty value. Skipping this update.`);
        return false;
    }
...
function mergeCollection(collectionKey, collection) {
    ...
    if (verifyCollection(collectionKey, collection)) {
        return Promise.resolve();
    }
...
/**
 * Overwrites a collection completely i.e. any collection members existing in the cache or storage not provided in the
 * collection passed to this method will be removed.
 *
 * @param {String} collectionKey
 * @param {Object} collection
 * @returns {Promise}
...
function setCollection(collectionKey, collection) {
    // Gracefully handle bad mergeCollection updates so it doesn't block the merge queue
    if (verifyCollection(collectionKey, collection, 'set')) {
        return Promise.resolve();
    }

    return getAllKeys().then((persistedKeys) => {
        // Get all existing keys that match the collection
        // eslint-disable-next-line rulesdir/prefer-underscore-method
        const existingCollectionKeys = persistedKeys.filter(key => isCollectionMemberKey(collectionKey, key));

        // From these get the keys that are not included in the set
        // eslint-disable-next-line rulesdir/prefer-underscore-method
        const keysInSetCollection = Object.keys(collection);
        const keysToRemove = _.difference(existingCollectionKeys, keysInSetCollection);
        const existingKeys = _.difference(existingCollectionKeys, keysToRemove);
        const newKeys = _.difference(keysInSetCollection, existingKeys);
        ...
        const promiseUpdate = Promise.all(_.map(existingKeys, get)).then(() => {
            ...
            // Let all subscribers know about dropped keys
...
function applyDecorators() {
    ...
    setCollection = decorate.decorateWithMetrics(set, 'Onyx:setCollection');
