 ...
 * @param {String} collectionKey
 * @param {Object} collection
 * @param {Boolean} dropExistingKeys
 * @returns {Promise}
 */
function setOrMergeCollection(collectionKey, collection, dropExistingKeys = false) {
    if (!_.isObject(collection) || _.isArray(collection) || _.isEmpty(collection)) {
        Logger.logInfo(`${dropExistingKeys ? 'set' : 'merge'}Collection() called with invalid or empty value. Skipping this update.`);
        return Promise.resolve();
    }
    ...
    return getAllKeys().then((persistedKeys) => {
        // In the case of setCollection() we want to drop the existing keys that are not included in our set. By setting them to null they will be removed.
        if (dropExistingKeys) {
            // eslint-disable-next-line rulesdir/prefer-underscore-method
            const existingKeysInStorage = persistedKeys.filter(key => isCollectionMemberKey(collectionKey, key));
            // eslint-disable-next-line rulesdir/prefer-underscore-method
            const keysToRemove = _.difference(existingKeysInStorage, Object.keys(collection));
        ...
        return Promise.all(promises)
            .catch((error) => evictStorageAndRetry(error, setOrMergeCollection, collection, dropExistingKeys))
            .then(() => promiseUpdate);
    });
}
...
function mergeCollection(collectionKey, collection) {
    return setOrMergeCollection(collectionKey, collection);
}
...
function setCollection(collectionKey, collection) {
    return mergeCollection(collectionKey, collection, true);
}
