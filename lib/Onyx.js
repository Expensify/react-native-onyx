...
const reduceCollectionWithSelector = (collection, selector) => _.reduce(collection, (finalCollection, item, key) => {
    const selectedValue = _.isFunction(selector)
        ? selector(item)
        : lodashGet(item, selector);
    ...
    for (let i = 0; i < stateMappingKeys.length; i++) {
        ...
        if (_.isFunction(subscriber.callback)) {
            ...
            if (isSubscribedToCollectionKey) {
                if (subscriber.waitForCollectionCallback) {
                    ...
                    if (subscriber.selector) {
                        const newData = _.reduce(cachedCollection, (finalData, cachedItem, cachedItemKey) => {
                            const selectedValue = lodashGet(cachedItem, subscriber.selector);

                            // If the selector didn't return any value, then don't add anything to our finalData because
                            // the subscriber doesn't want it
                            if (_.isUndefined(selectedValue)) {
                                return finalData;
                            }

                            return Object.assign(finalData, {[cachedItemKey]: selectedValue});
                        }, {});
                    ...
                    if (subscriber.reducer) {
                        const newData = _.reduce(cachedCollection, (finalData, cachedItem, cachedItemKey) => {
                            const selectedValue = subscriber.reducer(cachedItem);
                ...
                for (let j = 0; j < dataKeys.length; j++) {
                    ...
                    if (subscriber.selector) {
                        const newValue = lodashGet(newData, subscriber.selector);
                    ...
                    // If there is a reducer, only trigger the callback when that particular data has changed
                    if (subscriber.reducer) {
                        const newValue = subscriber.reducer(newData);
            ...
            if (isSubscribedToCollectionMemberKey) {
                ...
                if (subscriber.selector || subscriber.reducer) {
                    const newValue = subscriber.selector
                        ? lodashGet(cachedCollection[subscriber.key], subscriber.selector)
                        : subscriber.reducer(cachedCollection[subscriber.key]);
            ...
            if (isSubscribedToCollectionKey) {
                if (subscriber.selector || subscriber.reducer) {
                    subscriber.withOnyxInstance.setState((prevState) => {
                        const previousData = reduceCollectionWithSelector(prevState[subscriber.statePropertyName], subscriber.selector || subscriber.reducer);
                        const newData = reduceCollectionWithSelector(partialCollection, subscriber.selector || subscriber.reducer);
            ...
            if (isSubscribedToCollectionMemberKey) {
                ...
                if (subscriber.selector || subscriber.reducer) {
                    subscriber.withOnyxInstance.setState((prevState) => {
                        ...
                        const newData = subscriber.selector
                            ? lodashGet(cachedCollection[subscriber.key], subscriber.selector)
                            : subscriber.reducer(cachedCollection[subscriber.key]);
...
function keyChanged(key, data) {
    ...
    for (let i = 0; i < stateMappingKeys.length; i++) {
        ...
        if (_.isFunction(subscriber.callback)) {
            ...
            if (subscriber.selector) {
                const newValue = lodashGet(data, subscriber.selector);
                if (!_.isEqual(subscriber.lastCalledWith, newValue)) {
            ...
            if (subscriber.reducer) {
                const newValue = subscriber.reducer(data);
                if (!_.isEqual(subscriber.lastCalledWith, newValue)) {
                    subscriber.callback(newValue, key);
        ...
        if (subscriber.withOnyxInstance) {
            ...
            if (isCollectionKey(subscriber.key)) {
                ...
                if (subscriber.selector || subscriber.reducer) {
                    subscriber.withOnyxInstance.setState((prevState) => {
                        const prevData = prevState[subscriber.statePropertyName];
                        const newValue = subscriber.selector
                            ? lodashGet(data, subscriber.selector)
                            : subscriber.reducer(data);
                        const newData = {
                            [key]: newValue,
                        };
                        if (!_.isEqual(prevData, newData)) {
            ...
            if (subscriber.selector) {
                subscriber.withOnyxInstance.setState((prevState) => {
                    const previousValue = lodashGet(prevState, subscriber.selector);
                    const newValue = lodashGet(data, subscriber.selector);
            ...
            if (subscriber.reducer) {
                subscriber.withOnyxInstance.setState((prevState) => {
                    const previousValue = subscriber.reducer(prevState);
                    const newValue = subscriber.reducer(data);
                    if (!_.isEqual(previousValue, newValue)) {
                        return {
                            [subscriber.statePropertyName]: newValue,
 ...
 * @param {Function} [mapping.reducer]
...
function sendDataToConnection(mapping, val, matchedKey) {
    ...
    if (mapping.withOnyxInstance) {
        ...
        if (mapping.selector) {
            ...
            const newValue = lodashGet(val, mapping.selector);
        ...
        if (mapping.reducer) {
            if (isCollectionKey(mapping.key)) {
                const newValue = reduceCollectionWithSelector(val, mapping.reducer);
                PerformanceUtils.logSetStateCall(mapping, null, newValue, 'sendDataToConnection');
                mapping.withOnyxInstance.setWithOnyxState(mapping.statePropertyName, newValue);
                return;
            }

            const newValue = mapping.reducer(val);
            PerformanceUtils.logSetStateCall(mapping, null, newValue, 'sendDataToConnection');
            mapping.withOnyxInstance.setWithOnyxState(mapping.statePropertyName, newValue);
            return;
        }
    ...
    } else if (_.isFunction(mapping.callback)) {
        if (mapping.selector) {
            if (val) {
                mapping.callback(lodashGet(val, mapping.selector), matchedKey);
            }
            return;
        }
        if (mapping.reducer) {
            if (val) {
                mapping.callback(mapping.reducer(val), matchedKey);
            }
            return;
        }
 ...
 * @param {String} [mapping.selector] a path to a specific piece of onyx data stored on the key (same kind of selector that would be passed to lodash get())
 * @param {Function} [mapping.reducer] a function that will return a reduced copy of the item stored in Onyx (or an item in an Onyx collection). Use this to only subscribe to a subset of object properties
