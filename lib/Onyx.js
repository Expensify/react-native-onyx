 ...
 * @param {boolean} [notifyWithOnyxSubscribers=true]
 */
function keysChanged(collectionKey, partialCollection, notifyRegularSubscribers = true, notifyWithOnyxSubscribers = true) {
    // We are iterating over all subscribers similar to keyChanged(). However, we are looking for subscribers who are subscribing to either a collection key or
    // individual collection key member for the collection that is being updated. It is important to note that the collection parameter cane be a PARTIAL collection
    // and does not represent all the combined keys and values for a collection key. It is just the "new" data that was merged in via mergeCollection().
 ...
 * @param {Set} keysToOverwrite Set of collection item keys that should be overwritten without using the existing value
 * @returns {Promise}
 */
function mergeCollection(collectionKey, collection, keysToOverwrite) {
    ...
    return getAllKeys().then((persistedKeys) => {
        ...
        if (keyValuePairsForExistingCollection.length > 0) {
            promises.push(Storage.multiMerge(keyValuePairsForExistingCollection, keysToOverwrite));
        }
...
function update(data) {
    ...
    onyxCollectionKeySet.forEach((collectionKey) => {
        ...
        if (_.isEmpty(collectionItemKeys) || collectionItemKeys.length === 1) {
            // No keys of this collection are in the updateQueue. Skipping...
            // There is only one key in the collection. We should update it individually using `merge` or `set`. todo
            return;
        }
