...
function applyMerge(existingValue, changes) {
    ...
    if (_.isObject(existingValue) || _.isObject(lastChange)) {
        // We want to merge multiple object deltas together
        ...
        let numberOfObjectsAtTheEnd;
        if (_.every(changes, _.isObject)) {
            numberOfObjectsAtTheEnd = changes.length;
        } else {
            numberOfObjectsAtTheEnd = Utils.countObjectsAtEndOfArray(changes);
        }

        // Object values are merged one after the other
        // lodash adds a small overhead so we don't use it here
        // eslint-disable-next-line prefer-object-spread, rulesdir/prefer-underscore-method
        return _.reduce(changes.slice(-numberOfObjectsAtTheEnd), (modifiedData, change) => fastMerge(modifiedData, change),
            existingValue || {});
    }
    ...
    return get(key)
        .then((existingValue) => {
            try {
                ...
                // The batchedChanges is the result of all changes in the mergeQueue merged together.
                // If the mergeQueue contains nullish values (anywhere) and last value(s), it means that the user
                // intends to reset the existing value or remove the key
                // If there are other changes queued afterwards, we cannot simply merge the remaining changes
                // and merge it with the existingValue. Instead, we have to omit the existingValue and only set
                // the batchedChanges after any nullish values.
                // This only affects objects, because arrays and nullish values are always replaced by the newest value.
                const omitExistingValue = _.some(mergeQueue[key], change => _.isUndefined(change) || _.isNull(change));
                ...
                const modifiedData = Utils.removeNullObjectValues(applyMerge(omitExistingValue ? undefined : existingValue, [batchedChanges]));
