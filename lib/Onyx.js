...
/**
 * Merges all the values in the current merge queue
 *
 * @private
 * @param {String} key
 * @param {any} existingValue This parameter is just used to determine the type of the value
 * @returns {*}
 */
function mergeBatchedChanges(key, existingValue) {
    const changes = mergeQueue[key];
    delete mergeQueue[key];

    const lastChange = _.last(changes);

    if (_.isUndefined(existingValue) || _.every(changes, _.isUndefined) || _.isUndefined(lastChange)) {
        return undefined;
    }

    if (_.isArray(existingValue) || _.every(changes, _.isArray) || _.isArray(lastChange)) {
        return lastChange;
    }

    if (_.isObject(existingValue) || _.every(changes, _.isObject) || _.isObject(lastChange)) {
        return applyMerge({}, changes);
    }
...
function merge(key, changes) {
    ...
    return get(key)
        .then((existingValue) => {
            const batchedChanges = mergeBatchedChanges(key, existingValue);

            if (batchedChanges === null) { return Promise.resolve(); }
