 ...
 *
 * @param {Function} runModification
 * @param {String} key If "key" is not set (or set to ALL_KEYS_OPERATION) then the modification will be queued
 * until all prior modifications complete. After that, a new modification  with key -1 will be queued.
 * @returns {Promise}
 */
function queueModification(runModification, key = ALL_KEYS_OPERATION) {
    const currentTask = key === ALL_KEYS_OPERATION
        ? Promise.all(_.values(keyOperations))
        : Promise.all([
            keyOperations[ALL_KEYS_OPERATION]
            || Promise.resolve(),
            keyOperations[key],

        ]);

    keyOperations[key] = currentTask.finally(runModification);
    return keyOperations[key];
}
...
function remove(key) {
    ...
    return queueModification(nextModification, key);
}
 ...
 */
// eslint-disable-next-line rulesdir/no-negated-variables
function notify(key, modifiedData, method) {
    // Logging properties only since values could be sensitive things we don't want to log
    Logger.logInfo(`${method}() called for key: ${key}${_.isObject(modifiedData) ? ` properties: ${_.keys(modifiedData).join(',')}` : ''}`);
    ...
    if (!hasChanged) {
        notifySubscribersOnNextTick(key, modifiedData, subscriber => subscriber.initWithStoredValues === false);
        return;
    }

    notifySubscribersOnNextTick(key, modifiedData);
}
...
function set(key, value) {
    ...
    const nextModification = () => new Promise((resolve, reject) => {
        Storage.setItem(key, value)
            .then(resolve)
            .catch(error => evictStorageAndRetry(error, set, key, value).then(resolve).catch(reject));
    });
...
function applyMerge(existingValue, changes) {
    if (_.isUndefined(existingValue) || _.isNull(existingValue)) {
        return changes;
    }

    if (_.isArray(existingValue) || _.isArray(changes)) {
        return changes;
    }

    if (_.isObject(existingValue) || _.isObject(changes)) {
        // lodash adds a small overhead so we don't use it here
        // eslint-disable-next-line prefer-object-spread, rulesdir/prefer-underscore-method
        const newData = fastMerge(existingValue, changes);
 ...
 * behavior uses lodash/merge under the hood for `Object` and
 * for `Array`'s we simply replace the current value with the new one.
 *
...
function clear(keysToPreserve = []) {
    ...
    return queueModification(nextModification);
}
