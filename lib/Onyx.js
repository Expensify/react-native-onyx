...
/**
 * @param {Object} mapOnyxToState
 * @param {Component} component
 *
 * @returns {Promise}
 */
function connectReactComponent(mapOnyxToState, component) {
    const state = {};
    const mappings = {};
    const connectionIDs = {};
    return deferredInitTask.promise
        .then(getAllKeys)
        .then((keys) => {
            const keysToRetrieveFromOnyx = new Set();

            // Create mapping config + add to the connection map each thing
            _.each(mapOnyxToState, (mapping, statePropertyName) => {
                const connectionID = getNextConnectionID();
                const key = Str.result(mapping.key, component.props);
                connectionIDs[key] = connectionID;
                ...
                // No need to return this value in state if initWithStoredValues is false
                if (mapping.initWithStoredValues === false) {
                    return;
                }

                // Check to see if this key is flagged as a safe eviction key and add it to the recentlyAccessedKeys
                ...
                const matchingKeys = _.filter(keys, keyInStorage => isKeyMatch(mapping.key, keyInStorage));

                // If the key being connected to does not exist, initialize the value with null
                if (matchingKeys.length === 0) {
                    state[config.statePropertyName] = null;
                    return;
                }

                mappings[key] = {config, matchingKeys, connectionID};

                // Take all the matching keys and add them to our set so we can fetch them all at once
                _.each(matchingKeys, matchingKey => keysToRetrieveFromOnyx.add(matchingKey));
            });

            return AsyncStorage.multiGet([...keysToRetrieveFromOnyx]);
        })
        .then((values) => {
            const data = _.reduce(values, (prev, [key, value]) => {
                try {
                    const parsed = JSON.parse(value);
                    cache.set(key, parsed);
                    return {...prev, [key]: parsed};
                } catch (err) {
                    console.debug('[Onyx] Unable to parse storage key');
                }
                return prev;
            }, {});
            _.each(mappings, ({config, matchingKeys}) => {
                // React components must set their initial data as a single object when using collection keys.
                if (isCollectionKey(config.key)) {

                    const matchingValues = _.map(matchingKeys, matchingKey => data[matchingKey]);
                    const objectValue = _.reduce(matchingValues, (finalObject, value, i) => ({
                        ...finalObject,
                        [matchingKeys[i]]: value,
                    }), {});

                    state[config.statePropertyName] = objectValue;
                } else {
                    const matchingKey = _.first(matchingKeys);
                    state[config.statePropertyName] = data[matchingKey];
                }
            });
