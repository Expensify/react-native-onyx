...
function clear(keysToPreserve = []) {
    return getAllKeys()
        .then((keys) => {
            ...
            //  - Group 1: key/values which can be reset individually where every key has it's subscriber callback
            //      triggered. This will be any normal key as well as any collection member key where
            //      waitForCollectionCallback=false
            //  - Group 2: key/values that belong to a collection that has waitForCollectionCallback=true. These
            //      collection callbacks are expecting to only be called once, with the entire collection. For these
            //      keys, they must be grouped so that notifySubscribersOnNextTick() is called for the entire collection
            //      and not for each individual key. Otherwise the callback is called n times with each item in the
            //      collection and the callbacks won't handle the code properly.
            const keyValuesToResetIndividually = {};
            const keyValuesToResetAsEntireCollection = {};
            ...
            _.each(keyValuesToReset, (keyValue) => {
                ...
                // It's fine to go ahead and clear the cache here for everything, regarding of which group the key is in
                cache.set(key, value);

                // In order to group the key/values, we have to extract a possible collection key, then look for the
                // subscriber for that collection. When a subscriber is found, then the waitForCollectionCallback
                // is looked at and they key/values are grouped accordingly
                const collectionKey = key.substring(0, key.indexOf('_') + 1);
                if (collectionKey) {
                    const subscriber = _.findWhere(callbackToStateMapping, {key: collectionKey});
                    if (subscriber && subscriber.waitForCollectionCallback) {
                        if (!keyValuesToResetAsEntireCollection[collectionKey]) {
                            keyValuesToResetAsEntireCollection[collectionKey] = {};
                        }
                        keyValuesToResetAsEntireCollection[collectionKey][key] = value;
                        return;
                    }
                }
            ...
            // console.log(1, keyValuesToResetIndividually)
            // console.log(2, keyValuesToResetAsEntireCollection)

            // Notify the subscribers for each key/value group so they can receive the new values
            _.each(keyValuesToResetIndividually, (value, key) => {
                notifySubscribersOnNextTick(key, value);
            });
            _.each(keyValuesToResetAsEntireCollection, (value, key) => {
                notifySubscribersOnNextTick(key, value);
            });
