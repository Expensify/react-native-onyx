 ...
 * @param {Boolean} [options.shouldSyncMultipleInstances] Auto synchronize storage events between multiple instances of Onyx
...
function init({
    ...
    if (shouldSyncMultipleInstances) {
        Storage.useAutoInstanceSync(keyChanged);
    }
}

/**
 * Prepares a file so that it can be saved in Onyx.
 * Use this method to get an updated File reference.
 * Then save it as usual by Onyx.set or Onyx.merge
 * @param {File} file
 * @returns {File}
 *
 * @example
 * const preparedFile = Onyx.prepareFile(someFile);
 * Onyx.merge('someKey', {
...
function prepareFile(file) {
    return Storage.prepareFile(file);
}
