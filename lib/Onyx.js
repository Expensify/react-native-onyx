 ...
 * @param {String} key
 * @param {*} value
 * @param {Function} writeMethod
 * @returns {Promise}
 */
// eslint-disable-next-line rulesdir/no-negated-variables
function notifyAfterWrite(key, value) {
...
function set(key, value) {
    ...
    return new Promise(resolve => Storage.setItem(key, value)
        .then(updatedValue => resolve(
            notifyAfterWrite(key, updatedValue, Storage.mergeItem),
...
function hasPendingMergeForKey(key) {
    return Storage.hasPendingMergeForKey(key);
}
...
function merge(key, value) {
    // Logging properties only since values could be sensitive things we don't want to log
    Logger.logInfo(`emrge() called for key: ${key}${_.isObject(value) ? ` properties: ${_.keys(value).join(',')}` : ''}`);

    // Array values are always set never merged because the MergePatch algorithm in SQLite will always replace arrays.
    // Old concatenating of array values behavior is deprecated.
    if (_.isArray(value)) {
        return set(key, value);
    }

    return new Promise(resolve => get(key)
        .then(currentValue => Storage.mergeItem(key, value, currentValue)
            .then(updatedValue => resolve(
                notifyAfterWrite(key, updatedValue, Storage.mergeItem),
            )).catch(error => evictStorageAndRetry(error, merge, key, value))));
}
