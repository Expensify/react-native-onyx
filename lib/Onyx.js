 ...
 * @returns {Promise}
 */
function queueModification(key, runModification) {
    const currentTask = keyOperations[key] || Promise.resolve();
    keyOperations[key] = currentTask.then(runModification);
...
function remove(key) {
    ...
    return queueModification(key, nextModification);
}
