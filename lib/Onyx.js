 ...
 */
// eslint-disable-next-line rulesdir/no-negated-variables
function notify(key, modifiedData, isMerge = false) {
    // Logging properties only since values could be sensitive things we don't want to log
    Logger.logInfo(`${isMerge ? 'merge' : 'set'}() called for key: ${key}${_.isObject(modifiedData) ? ` properties: ${_.keys(modifiedData).join(',')}` : ''}`);
    ...
    if (!cache.hasValueChanged(key, modifiedData)) {
...
function set(key, value) {
    ...
    // We don't wait until the new data is written to storage.
    // Instead, we immediately notify subscribers and then return the new data
    const setPromise = Storage.setItem(key, value);

    return notify(key, value).then(() => setPromise).catch(error => evictStorageAndRetry(error, set, key, value));
}
...
function applyMerge(key, data) {
    ...
    if (_.isArray(data) || _.every(mergeValues, _.isArray)) { return _.last(mergeValues); }
    ...
    return get(key)
        .then((data) => {
            try {
                ...
                // We don't wait until the merge is written to storage.
                // Instead, we immediately notify subscribers and then return the merge promise
                const mergePromise = OnyxMerge.merge(key, changes, modifiedData);

                return notify(key, modifiedData, true).then(() => mergePromise);
            } catch (error) {
