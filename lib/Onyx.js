 ...
 */
function keyChanged(key, data, onlyUpdateInitWithStoredValuesFalse = false) {
    ...
    for (let i = stateMappingKeys.length; i--;) {
        const subscriber = callbackToStateMapping[stateMappingKeys[i]];
        if (!subscriber || !isKeyMatch(subscriber.key, key) || (onlyUpdateInitWithStoredValuesFalse && subscriber.initWithStoredValues !== false)) {
            continue;
        }
