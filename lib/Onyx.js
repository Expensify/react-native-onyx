...
/**
 * Note: This method should only be used internally in withOnyx and is designed to prevent unnecessary
 * commits when a wrapped component is requesting cached values. In this case, it should be able to get all
 * values synchronously if they exist in the cache.
 *
 * @param {Object} mapOnyxToState
 * @param {React.Component} component
 * @param {Object} props
 */
function dangerouslyGetCachedStateSync(mapOnyxToState, component, props = {}) {
    const state = {};
    const connectionIDs = {};
    let cacheMiss = false;
    ...
    _.each(mapOnyxToState, (mapping, statePropertyName) => {
        // Stop building state if we are unable to locate a cached value
        if (cacheMiss) {
            return;
        }
        ...
        if (config.initWithStoredValues === false) {
            return connectionID;
        }

        // Find all the keys matched by the config key
        const matchingKeys = _.filter(allCacheKeys, key => isKeyMatch(config.key, key));
        if (matchingKeys.length === 0) {
            cacheMiss = true;
            return;
        }
        ...
        if (isCollectionKey(key)) {
            const data = {};
            _.each(matchingKeys, (matchingKey) => {
                if (!cache.hasCacheForKey(matchingKey)) {
                    cacheMiss = true;
                    return;
                }
                data[matchingKey] = cache.getValue(matchingKey);
            });
            state[statePropertyName] = data;
        } else {
            if (!cache.hasCacheForKey(key)) {
                cacheMiss = true;
                return;
            }
            state[statePropertyName] = cache.getValue(key);
        }
    });

    return {
        state: cacheMiss ? false : state,
        connectionIDs,
    };
}
