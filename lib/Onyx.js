 ...
 */
// eslint-disable-next-line rulesdir/no-negated-variables
function notify(key, modifiedData, writeMethod = 'set') {
    // Logging properties only since values could be sensitive things we don't want to log
    Logger.logInfo(`${writeMethod}() called for key: ${key}${_.isObject(modifiedData) ? ` properties: ${_.keys(modifiedData).join(',')}` : ''}`);
