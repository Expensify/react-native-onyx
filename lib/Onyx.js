 ...
 *
 * @param {Object} data An object containing the Onyx data to set
 * @param {String} data.onyxMethod One of [set, merge] Onyx methods
 * @param {String} data.key ONYXKEY to set
 * @param {*} data.value Value to store in ONYXKEY
 */
function update(data) {
    // First, validate the Onyx object is in the format we expect
    _.each(data, ({onyxMethod, key, value}) => {
        if (!_.inArray(onyxMethod, ['set', 'merge'])) {
            throw new Error(`Invalid method ${onyxMethod} in Onyx update.`);
        }
        if (!_.isString(key)) {
            throw new Error(`Invalid ${key} key provided in Onyx update. Onyx key must be of type string.`);
        }
        if (typeof value === 'undefined') {
            throw new Error(`Undefined value passed to key ${key} in Onyx update.`);
        }
    });
