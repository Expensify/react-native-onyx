...
function mergeCollection(collectionKey, collection) {
    ...
    let collectionKeyCheckPassed = true;
    _.each(collection, (_data, dataKey) => {
        if (isKeyMatch(collectionKey, dataKey)) {
            return;
        }

        Logger.logInfo(`Provided collection doesn't have all its data belonging to the same parent. CollectionKey: ${collectionKey}, DataKey: ${dataKey}`);
        collectionKeyCheckPassed = false;
    });
    if (!collectionKeyCheckPassed) {
        return Promise.resolve();
    }
