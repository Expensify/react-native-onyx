...
function findKeyInState(key) {
    if (!callbackToStateMapping[key] || _.isEmpty(callbackToStateMapping[key])) {
        return;
    }

    const subscriber = _.find(callbackToStateMapping[key], subscriber => subscriber.withOnyxInstance);

    if (!subscriber) {
        return;
    }

    return lodashGet(subscriber.withOnyxInstance, ['state', subscriber.statePropertyName]);
}
 ...
 */
function get(key) {
    const keyInStateValue = findKeyInState(key);

    if (keyInStateValue) {
        return new Promise((resolve) => resolve(keyInStateValue));
    }
...
function keysChanged(collectionKey, collection) {
    ...
    _.each(callbackToStateMapping, (subscribers) => {
...
function connect(mapping) {
    ...
    if (!callbackToStateMapping[mapping.key]) {
        callbackToStateMapping[mapping.key] = {};
    }

    callbackToStateMapping[mapping.key][connectionID] = mapping;
 ...
 */
function disconnect(key, connectionID, keyToRemoveFromEvictionBlocklist) {
    if (!callbackToStateMapping[key]) {
        return;
    }

    if (!callbackToStateMapping[key][connectionID]) {
        return;
    }
    ...
    delete callbackToStateMapping[key][connectionID];

    if (_.isEmpty(callbackToStateMapping[key])) {
        delete callbackToStateMapping[key];
    }
}
...
/**
 * Do we have verbose logging set up?
 *
 * @returns {Boolean}
 */
function isDebug() {
...
function init({
    ...
    if (debug) {
        startTime = global.performance.now();
    }
}

function recordOnyxFinishedWork() {
    const time = global.performance.now() - startTime - 1000;
    console.debug(`[Onyx] finished work in ${time.toFixed(2)}`);
}
...
const Onyx = {
    ...
    isSafeEvictionKey,
    isDebug,
    recordOnyxFinishedWork: _.debounce(recordOnyxFinishedWork, 1000),
};
