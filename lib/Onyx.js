 ...
 * @returns {Promise<Boolean>} Whether there was a problem initializing the platform storage provider and Onyx uses the fallback provider instead.
 * @example
 * Onyx.init({
 *     keys: ONYXKEYS,
 *     initialKeyStates: {
...
function init({
    ...
    return new Promise((resolve) => storageInitPromise.then(() => resolve(true)).catch(() => resolve(false)));
}

function isUsingFallbackStorageProvider() {
    const storageProviders = Storage.getStorageProviders();

    return storageProviders.length === 1 && storageProviders[0] === MemoryOnlyProvider;
}
...
const Onyx = {
    ...
    getAllKeys,
    init,
    isUsingFallbackStorageProvider,
