...
function get(key) {
    ...
    return cache.resolveTask('get', () => AsyncStorage.getItem(key)
        .then((val) => {
...
function multiSet(data) {
    // Todo: use _.pairs for the same result
    ...
    // Capture any changes to cache
    cache.merge(keyValuePairs);
...
function mergeCollection(collectionKey, collection) {
    ...
    return getAllKeys()
        .then((keys) => {
            ...
            // Capture any changes to cache
            cache.merge(keyValuePairsForExistingCollection);
            cache.merge(keyValuePairsForNewCollection);
