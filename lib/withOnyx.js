...
export default function (mapOnyxToState) {
    return (WrappedComponent) => {
        class withOnyx extends React.Component {
            ...
            /**
             * Using the next set of props look to see which keys need to be remapped and then re-subscribe them to Onyx. This is necessary so that a
             * parent prop change will only trigger a single re-render of the wrapped component rather than one update for the prop change followed by
             * another update when the connection sets state again.
             *
            ...
            checkAndUpdateLoading() {
                ...
                if (_.every(requiredKeysForInit, key => !this.loadingKeys[key])) {
