    ...
    return (WrappedComponent) => {
        class withOnyx extends React.Component {
            constructor(props) {
                ...
                // If we are in debug mode then show the name of the component and record the start time.
                if (Onyx.isDebug()) {
                    console.debug(`[withOnyx] ${getDisplayName(WrappedComponent)} constructed`);
            ...
            componentDidUpdate(prevProps) {
                ...
                _.each(mapOnyxToState, (mapping, propertyName) => {
                    ...
                    if (previousKey !== newKey) {
                        Onyx.disconnect(previousKey, this.activeConnectionIDs[previousKey], previousKey);
            ...
            componentWillUnmount() {
                ...
                _.each(mapOnyxToState, (mapping) => {
                    ...
                    Onyx.disconnect(key, connectionID, key);
                });
            }
            ...
            checkAndUpdateLoading() {
                ...
                if (_.every(requiredKeysForInit, key => !_.isUndefined(this.state[key]))) {
                    if (Onyx.isDebug()) {
                        const time = global.performance.now() - this.startTime;
                        console.debug(`[withOnyx] ${getDisplayName(WrappedComponent)} all keys available in ${time.toFixed(2)}ms`);
                        Onyx.recordOnyxFinishedWork();
                    }
                    Onyx.recordOnyxFinishedWork();
