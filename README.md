...
`bench:report` automatically detects noisy results (high RME, extreme outliers) and re-runs the affected benchmark files to get cleaner data. This typically stabilizes within 1-2 passes. Criteria:
- RME > 50% on operations with mean > 1ms
- Extreme outlier spike that shifts the mean by >2ms and >20%

To skip stabilization (faster but noisier): `./scripts/benchAndReport.sh --no-stabilize`

To control retry limits: `./scripts/benchAndReport.sh --max-retries 5`
